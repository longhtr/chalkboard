...
describe('API response decoders', () => {
  it('distinguishes withdrawn board access from transient failures', () => {
    expect(isBoardAccessWithdrawnError(new ApiError('Forbidden', 403))).toBe(
      true,
    );
    expect(isBoardAccessWithdrawnError(new ApiError('Missing', 404))).toBe(
      true,
    );
    expect(
      isBoardAccessWithdrawnError(new ApiError('Server failure', 500)),
    ).toBe(false);
    expect(isBoardAccessWithdrawnError(new ApiError('Offline', null))).toBe(
      false,
    );
  });
