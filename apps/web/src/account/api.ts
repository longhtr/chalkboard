...
/** One pending offer of access, as the invitee sees it. */
  ...
  constructor(
    ...
    const requestId =
      ...
      REQUEST_ID_PATTERN.test(options.requestId)
        ? options.requestId
        : null;
    // A 4xx is an ordinary answer the reader can act on: the address is taken,
    // a change is already pending. Quoting a support reference there dresses a
    // normal refusal up as a fault. Anything else — a server failure, or a
    // success whose body violated its contract — is worth correlating.
    const routineRefusal = status !== null && status >= 400 && status < 500;
    super(
      requestId === null || routineRefusal
        ? message
...
/**
 * Validates whichever of the two outcomes sharing by email produced.
 *
 * Somebody already on the board is re-roled and comes back as a member;
 * somebody new is offered an invitation and is not a member yet. The caller
 * has to tell them apart, because only one has changed who can open the board.
 */
export function decodeShareResponse(
  value: unknown,
):
  | { invitation: BoardInvitation; kind: 'invitation' }
  | { kind: 'member'; member: BoardMember } {
  const response = object(value);
  if (response.member !== undefined) {
    return { kind: 'member', member: decodeBoardMember(response.member) };
  }
  return {
    invitation: decodeInvitation(response.invitation),
    kind: 'invitation',
  };
}
...
/** Validates the account's own board-invitation preference. */
export function decodeBoardInvitationPreference(value: unknown): {
  acceptsBoardInvitations: boolean;
} {
  const response = object(value);
  if (typeof response.acceptsBoardInvitations !== 'boolean') {
    throw new TypeError(INVALID_RESPONSE_MESSAGE);
  }
  return { acceptsBoardInvitations: response.acceptsBoardInvitations };
}

/** Validates the owner's view of outstanding offers on one board. */
export function decodePendingInvitationsResponse(value: unknown): {
  invitations: BoardPendingInvitation[];
} {
  return {
    invitations: array(object(value).invitations, (item) => {
      const invitation = object(item);
      const role = boardRole(invitation.role);
      if (role === 'owner') throw new TypeError(INVALID_RESPONSE_MESSAGE);
      return {
        displayName: string(invitation.displayName),
...
/** Validates a pending-invitation list response and every invitation. */
export function decodeInvitationsResponse(value: unknown): {
  invitations: BoardInvitation[];
} {
  return {
    invitations: array(object(value).invitations, decodeInvitation),
  };
}

function decodeInvitation(value: unknown): BoardInvitation {
  const invitation = object(value);
  const role = boardRole(invitation.role);
  if (role === 'owner') throw new TypeError(INVALID_RESPONSE_MESSAGE);
  return {
    boardId: string(invitation.boardId),
    invitedAt: string(invitation.invitedAt),
    invitedByDisplayName: string(invitation.invitedByDisplayName),
    role,
...
/** A board is definitively absent from this account, rather than temporarily unreadable. */
export function isBoardAccessWithdrawnError(error: unknown): boolean {
