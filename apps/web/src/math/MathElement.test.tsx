...
describe('MathElement', () => {
  ...
  it('rebuilds and redecorates the hidden static tree when editing ends', () => {
    ...
    if (!(hiddenContent instanceof HTMLElement)) {
      throw new Error('Expected hidden rendered mixed-text content');
    }
    hiddenContent.replaceChildren(document.createTextNode('stale'));
    ...
    expect(container.querySelectorAll('.mixed-text-line-break')).toHaveLength(
      1,
    );
    expect(
      container.querySelector('.mixed-text-element__content'),
