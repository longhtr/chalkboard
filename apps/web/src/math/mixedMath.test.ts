...
describe('mixed math source', () => {
  it('classifies one formula as math-only without discarding row structure', () => {
    expect(isMathOnlyMixedSource('$x$')).toBe(true);
    expect(isMathOnlyMixedSource('  $x$\t')).toBe(true);
    expect(isMathOnlyMixedSource('$x$\n')).toBe(false);
    expect(isMathOnlyMixedSource('\n$x$')).toBe(false);
    expect(isMathOnlyMixedSource('$x$\n\n')).toBe(false);
    expect(isMathOnlyMixedSource('$x$ $y$')).toBe(false);
    expect(isMathOnlyMixedSource('before $x$')).toBe(false);
  });
  ...
  it('canonicalizes nested editor newlines for delimited and math-only values', () => {
    ...
    // A row boundary at top-level is between formulas, not inside an unfinished
    // argument. Moving it after the whole segment deletes the two delimiters at
    // the boundary and collapses both rows into one formula.
    expect(
      canonicalizeMathLiveEditorValue(
        `$=\\frac{e^{-z}}{1+e^{-z}}\\text{}${String.fromCodePoint(0x2063)}=\\left(-\\frac{1}{1+e^{-z}}\\right)\\left(-e^{-z}\\right)$`,
        { wrapUndelimitedMath: false },
      ),
    ).toBe(
      '$=\\frac{e^{-z}}{1+e^{-z}}\\text{}$\n$=\\left(-\\frac{1}{1+e^{-z}}\\right)\\left(-e^{-z}\\right)$',
    );
  });
