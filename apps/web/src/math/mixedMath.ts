...
/** Invisible editor sentinel representing a literal prose `{`. */
export const MATHLIVE_LITERAL_BRACE_LEFT = '\u206a';
/** Invisible editor sentinel representing a literal prose `}`. */
export const MATHLIVE_LITERAL_BRACE_RIGHT = '\u206b';
/** Invisible editor sentinel representing a literal prose `%`. */
export const MATHLIVE_LITERAL_PERCENT = '\u206c';
/**
 * Invisible editor sentinel representing an undelimited prose `$`.
 *
 * Distinct from {@link MATHLIVE_LITERAL_DOLLAR} only so the two decode back to
 * what each was written as: canonical `\$` stays escaped, while a `$` that
 * closes nothing stays plain rather than having an escape added to source the
 * writer never asked to change.
 */
export const MATHLIVE_BARE_DOLLAR = '\u206d';
...
// A text-mode colour reaches MathLive as a single invisible marker character,
// never as a `\textcolor` command.
//
// The editable field is the reason. `convertLatexToMarkup` understands
// `\textcolor` in either mode, but the editable field understands it only in
// math mode: given one in text mode it escapes the command into literal
// characters, so reopening a coloured block showed the reader
// `A\textcolor{#ff8787}{R}` as prose. Markers avoid the command entirely.
//
// Markers used to be a fixed table of five colours, which is the same defect
// wearing a palette: every dark-theme colour and every colour a reader picks
// had no marker, so its command survived expansion and was drawn literally.
// Markers are therefore allocated per colour as colours are met. They never
// reach storage, which keeps canonical `\textcolor` commands, so an assignment
// only has to hold within one session.
//
// Every code point below is intrinsically zero-width, so a marker cannot flash
// as a glyph before decoration runs, and every one is a single code point, so
// one marker is one MathLive atom and one caret offset.
const MARKER_RANGES: ReadonlyArray<readonly [number, number]> = [
  // The original five, kept first so the common palette colours keep the
  // markers they have always had.
  [0x200b, 0x200c],
  [0x2060, 0x2062],
  // Variation selectors supplement, then the tags block: invisible, assigned,
  // and used for nothing else here.
  [0xe0100, 0xe01ef],
  [0xe0020, 0xe007f],
];

function markerRangeCodePoints(): number[] {
  const points: number[] = [];
  for (const [first, last] of MARKER_RANGES) {
    for (let point = first; point <= last; point += 1) points.push(point);
  }
  return points;
}

const markerPool = markerRangeCodePoints().map((point) =>
  String.fromCodePoint(point),
);

/**
 * Character-class body matching any colour marker, for splitting a text run.
 *
 * Derived from the pool rather than written beside it: a hand-copied class that
 * had fallen behind the pool is what left later markers unsplit, so their
 * colour was silently never applied.
 */
function characterClassBody(
  ...
  return ranges
    .map(([first, last]) =>
      first === last
        ...
        : `\\u{${first.toString(16).toUpperCase()}}-\\u{${last.toString(16).toUpperCase()}}`,
    )
    .join('');
}

export const TEXT_COLOR_MARKER_CLASS = characterClassBody(MARKER_RANGES);

/**
 * Every code point this module reads as a sentinel rather than as writing:
 * the color pool above, and the contiguous run of line-break, literal and
 * style sentinels declared at the top of this file.
 */
const RESERVED_RANGES: ReadonlyArray<readonly [number, number]> = [
  ...MARKER_RANGES,
  [
    MATHLIVE_LINE_BREAK.codePointAt(0) ?? 0,
    MATHLIVE_BARE_DOLLAR.codePointAt(0) ?? 0,
  ],
];

const reservedPattern = new RegExp(
  `[${characterClassBody(RESERVED_RANGES)}]`,
  'gu',
);

/**
 * Removes characters this module reserves, for text arriving from outside.
 *
 * The sentinels are real, assigned code points that occur in real writing: the
 * tags block carries subdivision flag emoji, and the variation selectors carry
 * CJK ideographic variation sequences. Pasting either would otherwise be read
 * as a color change, a line break, or a literal brace. They are invisible and
 * carry no meaning of their own, so dropping them loses the reader nothing.
 */
export function stripReservedSentinels(value: string): string {
  return value.replace(reservedPattern, '');
}

/**
 * Each sentinel that stands for writing, beside how source spells it.
 *
 * One table so the correspondence has a single owner. It decodes editor values
 * back to source below, and the caret mapping reads it to give both spellings
 * of a document the same token identities; a sentinel added here reaches both
 * without either restating it. They diverged once, and rows spelled `\n` in
 * source and a sentinel in the field stopped aligning, which left a third of
 * the caret positions in an ordinary derivation impossible to reach.
 *
 * Sentinels absent from this table stand for formatting rather than writing --
 * the style and colour markers -- and are dropped rather than spelled.
 */
export const MATHLIVE_SENTINEL_SPELLINGS: ReadonlyMap<string, string> = new Map(
  [
    [MATHLIVE_LINE_BREAK, '\n'],
...
const reservedCharacters = new Set<string>(
  RESERVED_RANGES.flatMap(([first, last]) => {
...
/** Reports whether one character is reserved rather than writing. */
export function isReservedSentinel(character: string): boolean {
  return reservedCharacters.has(character);
}

const SEEDED_TEXT_COLORS = [
  '#1f2937',
  '#e03131',
  '#1971c2',
  '#2f9e44',
...
const colorByMarker = new Map<string, string>();
const markerByColor = new Map<string, string>();
let allocatedMarkers = 0;

function markerForColor(color: string): string | undefined {
  const existing = markerByColor.get(color);
  if (existing !== undefined) return existing;
  const marker = markerPool[allocatedMarkers];
  if (marker === undefined) return undefined;
  allocatedMarkers += 1;
  markerByColor.set(color, marker);
  colorByMarker.set(marker, color);
  return marker;
}

for (const color of SEEDED_TEXT_COLORS) markerForColor(color);
...
export function expandTextColors(source: string, baseColor: string): string {
  const transform = (value: string, inheritedColor: string): string => {
    ...
    while (cursor < value.length) {
      ...
      const colorMarker = markerForColor(color);
      const inheritedMarker = markerForColor(inheritedColor);
      // Only an exhausted pool leaves a command behind. Passing it through
      // keeps the source lossless; the static renderer still colours it, and
      // the editable field still shows it as prose.
...
/**
 * Replaces prose characters LaTeX consumes as syntax with editor sentinels.
 *
 * Canonical source stores a written `{`, `}` or `%` as itself, but MathLive
 * reads them as syntax: braces open and close a group and leave nothing behind,
 * and `%` comments out the rest of the row. Writing `a{b}c` and reopening the
 * board therefore showed `abc`, and a single `%` silently deleted everything
 * after it. `$` and `\` already had sentinels for exactly this reason; these are
 * the same idea, applied to the characters that were still being eaten.
 *
 * Braces that belong to a command stay as they are, so `\textbf{bold}` keeps
 * working. Each brace is paired with its own opener, so a literal brace inside a
 * command argument cannot be mistaken for that argument's end.
 */
function markLiteralTextCharacters(text: string): string {
  const openBraces: Array<'literal' | 'syntax'> = [];
  ...
  while (cursor < text.length) {
    ...
    if (character === '\\') {
      const command = /^\\(?:[a-zA-Z]+\*?|[\s\S])/u.exec(
        text.slice(cursor),
      )?.[0];
      if (command === undefined) break;
      result += command;
      cursor += command.length;
      expectingArgument = /[a-zA-Z*]$/u.test(command);
      continue;
    }
    if (character === '{') {
      const kind = expectingArgument ? 'syntax' : 'literal';
      openBraces.push(kind);
      result += kind === 'syntax' ? character : MATHLIVE_LITERAL_BRACE_LEFT;
      expectingArgument = false;
      cursor += 1;
      continue;
    }
    if (character === '}') {
      // An unmatched `}` is literal: nothing it could close was ever opened.
      result +=
        openBraces.pop() === 'syntax'
          ? character
          : MATHLIVE_LITERAL_BRACE_RIGHT;
      cursor += 1;
      continue;
    ...
    if (character === '%') {
      result += MATHLIVE_LITERAL_PERCENT;
      cursor += 1;
      continue;
    }
    if (character === '$') {
      // `parseMixedText` already claimed every delimiter, so a `$` still inside
      // a text segment closes nothing. Left bare it puts MathLive into math
      // mode, which is how forgetting a closing `$` silently reset the rest of
      // the block into italics.
      result += MATHLIVE_BARE_DOLLAR;
      cursor += 1;
      continue;
    }
    result += character;
    if (!/\s/u.test(character)) expectingArgument = false;
    cursor += 1;
  }
    ...
    .map((segment) =>
      segment.kind === 'text'
        ? toMathLiveMultilineSource(
            markLiteralTextCharacters(
              normalizeMathLiveTextSource(segment.source).replaceAll(
                '\\$',
                MATHLIVE_LITERAL_DOLLAR,
              ),
            ),
...
/** Decodes the sentinels that stand for writing back into what source writes. */
export function fromMathLiveMultilineSource(source: string): string {
  let result = '';
  for (const character of source) {
    result += MATHLIVE_SENTINEL_SPELLINGS.get(character) ?? character;
  }
  return result;
}
...
export function liftMathLineBreaks(source: string): string {
  ...
  for (const segment of segments) {
    result += source.slice(cursor, segment.openStart);
    const delimiter = source.slice(segment.openStart, segment.contentStart);
    const rows: { latex: string; nestedBreaks: number }[] = [
      { latex: '', nestedBreaks: 0 },
    ];
    let braceDepth = 0;
    for (let index = 0; index < segment.latex.length; index += 1) {
      const character = segment.latex[index] ?? '';
      const row = rows.at(-1);
      if (row === undefined) continue;
      if (character === '\n') {
        if (braceDepth === 0) rows.push({ latex: '', nestedBreaks: 0 });
        else row.nestedBreaks += 1;
        continue;
      }
      row.latex += character;
      if (isEscaped(segment.latex, index)) continue;
    ...
    result += rows
      .map(
        (row) =>
          `${delimiter}${row.latex || ' '}${delimiter}${'\n'.repeat(row.nestedBreaks)}`,
      )
      .join('\n');
 ...
 */
/** Trims spaces and tabs while keeping rows, which are content rather than padding. */
...
export function mixedSourceFromMathLiveEditor(
  ...
  const canonical = canonicalizeMathLiveEditorValue(valueWithoutPlaceholders, {
    wrapUndelimitedMath:
      !options.retainsMathOnlySource &&
      options.hasExplicitMath &&
      valueWithoutPlaceholders.includes(MATHLIVE_LINE_BREAK),
  });
  // Colour is applied to a selection, and in the editor a maths region is
  // delimited by ordinary `$` characters, so colouring one wraps the delimiters
  // too: `\textcolor{c}{$x$}`. Reduce colour to markers before the maths
  // normalizer runs, or it reads those delimiters as nested maths, rearranges
  // the segments, and the colour is lost - or survives as literal text.
  const value = expandTextColors(canonical, options.baseColor);

  let normalized: string;
  if (options.retainsMathOnlySource) {
    // `liftMathLineBreaks` moves rows out of the maths and leaves them trailing
    // the segment, so the value arrives here already delimited. Wrapping it a
    // second time produced `$$…$$`, and trimming then deleted every row that
    // had just been lifted: a block whose rows were separated by blank lines
    // collapsed to a single line, and stayed collapsed because the loss was
    // published.
    normalized =
      mathSegments(value).length > 0
        ? normalizeMathLiveSource(value)
        : `$${trimHorizontalWhitespace(value) || ' '}$`;
...
/** Reports one formula with no prose or row structure outside its delimiters. */
export function isMathOnlyMixedSource(source: string): boolean {
  const segments = parseMixedText(source);
  return (
    segments.filter((segment) => segment.kind === 'math').length === 1 &&
    segments.every(
      (segment) =>
        segment.kind === 'math' ||
        (!segment.source.includes('\n') && segment.source.trim() === ''),
    )
  );
}
