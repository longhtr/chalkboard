...
import {
  decorateExcalifontLayout,
  EXCALIFONT_OPERATOR_LAYOUT_CSS,
} from './excalifontLayout';
...
describe('bar correction', () => {
  it('does not stretch an evaluation bar beyond MathLive’s fitted height', () => {
    const evaluationRule = EXCALIFONT_OPERATOR_LAYOUT_CSS.match(
      /\[data-excalifont-delim='bar'\] \{[^}]+\}/u,
    )?.[0];

    expect(evaluationRule).toContain('transform: scaleY(1);');
  });
