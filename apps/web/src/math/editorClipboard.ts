...
export function editorClipboardInsertions(
  ...
  // Text from outside carries no sentinels of ours, and any code point that
  // looks like one is the writer's own character rather than an instruction.
  const lines = stripReservedSentinels(value)
    .replace(/\r\n?/g, '\n')
    .split('\n');
  return {
