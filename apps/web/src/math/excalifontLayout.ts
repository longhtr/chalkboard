...
// MathLive already sizes an evaluation delimiter to the fraction or expression
// it closes. Stretching that finished delimiter again made `\right|` nearly
// twice as tall as a fraction, so preserve MathLive's fitted height.
const EVALUATION_BAR_SCALE = '1';
// Text mode and math mode both use `bar-plain`, but not the same glyph. A text
// bar comes from Excalifont's full-height text face; a short norm or
// absolute-value bar comes from KaTeX_Main's much shorter Excalifont math
// glyph. Keep the already-tall text stroke where it is and bring only a short
// math stroke up to the same painted height. MathLive's stacked and manually
// sized delimiters already reach around what they enclose, so they retain the
// smaller correction instead of doubling once more.
const SHORT_MATH_BAR_SELECTOR =
  "[data-excalifont-delim='bar-plain']:not(.ML__text):not(.ML__delim-mult):not([class*='ML__delim-size'])";
const SHORT_MATH_BAR_SCALE = '2.1';
const SIZED_MATH_BAR_SELECTOR =
  "[data-excalifont-delim='bar-plain']:is(.ML__delim-mult, [class*='ML__delim-size'])";
const SIZED_MATH_BAR_SCALE = '1.12';
const TEXT_BAR_SELECTOR = "[data-excalifont-delim='bar-plain'].ML__text";
const TEXT_BAR_SCALE = '1.12';
...
// The arrow is a combining mark, so MathLive centres a box with no advance
// width and the ink lands to the right of the letter. How far to pull it back
// is measured against where the spacing accents sit, not against the letter's
// exact middle: every accent is set a little right of that over an italic.
const VECTOR_ACCENT_SHIFT = '-0.22em';
const VECTOR_ACCENT_RISE = '-0.02em';
 ...
 * evaluation bound with its own calibrated stretch. Ordinary math bars are
 * brought up to the painted height of an ordinary text bar, while text bars
...
${SHORT_MATH_BAR_SELECTOR} {
  display: inline-block;
  transform: scaleY(${SHORT_MATH_BAR_SCALE});
}

${SIZED_MATH_BAR_SELECTOR} {
  display: inline-block;
  transform: scaleY(${SIZED_MATH_BAR_SCALE});
}

${TEXT_BAR_SELECTOR} {
  display: inline-block;
  transform: scaleY(${TEXT_BAR_SCALE});
}
...
${VECTOR_ACCENT_SELECTOR} {
  left: ${VECTOR_ACCENT_SHIFT};
  top: ${VECTOR_ACCENT_RISE};
}
...
:host([data-workspace-font='excalifont']) ${SHORT_MATH_BAR_SELECTOR} {
  display: inline-block;
  transform: scaleY(${SHORT_MATH_BAR_SCALE});
}

:host([data-workspace-font='excalifont']) ${SIZED_MATH_BAR_SELECTOR} {
  display: inline-block;
  transform: scaleY(${SIZED_MATH_BAR_SCALE});
}

:host([data-workspace-font='excalifont']) ${TEXT_BAR_SELECTOR} {
  display: inline-block;
  transform: scaleY(${TEXT_BAR_SCALE});
}
