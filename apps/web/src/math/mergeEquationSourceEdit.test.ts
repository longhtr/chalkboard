...
describe('mergeEquationSourceEdit', () => {
  it('preserves concurrent insertions at the same source boundary', () => {
    ...
    expect(
      mergeEquationSourceEdit('Shared $ x $', 'Shared $ x $B', 'Shared $ xq $'),
    ).toBe('Shared $ xq $B');
    expect(
      mergeEquationSourceEdit('Shared $ x $', 'Shared $ xq $', 'Shared $ x $B'),
    ).toBe('Shared $ xq $B');
  });

  it('does not duplicate an insertion that current already contains', () => {
    expect(mergeEquationSourceEdit('ac', 'abc', 'abxc')).toBe('abxc');
    expect(mergeEquationSourceEdit('a', 'aa', 'baa')).toBe('baa');
    expect(mergeEquationSourceEdit('a', 'aa', 'ba')).toBe('baa');
    expect(
