...
function MathElementComponent({
  ...
  const isMathOnly = useMemo(
    () => isMathOnlyMixedSource(renderedElement.source),
    [renderedElement.source],
  );
  ...
  // markup on every camera frame and threw the decoration away with it. Keep
  // the object steady while the markup is unchanged. The content node's phase
  // key below deliberately rebuilds once when the hidden static tree becomes
  // visible, so a late active-editor render cannot leave stale undecorated
  // markup on screen.
  ...
  // changes and the active-to-static handoff, the decorated DOM survives, and
  // the camera -- a fresh object on every pan and zoom -- no longer drags the
  // tagging walk, the sentinel split, the colour pass and the clearance
