...
export function applyLineClearance(root: ParentNode, scale: number): void {
  ...
  // Everything a break span sits inside. Selecting across lines makes MathLive
  // wrap the selected run in one element, and that wrapper's box reaches from
  // the first selected line to the last. Counted as ink on the line it starts
  // on, it read as that line overflowing by the height of the whole block, and
  // select-all pushed the second line most of a page down. A wrapper is not
  // ink; the writing inside it is measured on its own.
  const spanning = new Set<Element>();
  for (const node of breaks) {
    let ancestor = node.parentElement;
    while (ancestor !== null && ancestor !== base) {
      spanning.add(ancestor);
