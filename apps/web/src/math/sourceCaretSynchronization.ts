...
interface SourceCaretSynchronizationOptions {
  ...
  onSourceOffset?(sourceOffset: number): void;
}

export interface SourceCaretSynchronization {
  dispose(): void;
  fieldOffsetForSource(sourceOffset: number, source?: string): number;
  invalidate(): void;
}
...
export function installSourceCaretSynchronization({
  field,
  getFieldOffset = () => field.position,
  getSource,
  onFieldOffset,
  onSourceOffset,
}: SourceCaretSynchronizationOptions): SourceCaretSynchronization {
  ...
  return {
    dispose() {
      ...
      field.removeEventListener(
        'chalkboard-source-caret-request',
        handleRequest,
      );
    },
    fieldOffsetForSource(sourceOffset: number, source = getSource()) {
      return fieldOffsetForSourceOffset(
        source,
        boundaries.read(),
        sourceOffset,
      );
