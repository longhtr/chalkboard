  ...
  useEffect(() => {
      ...
      if (!positionInitialized) {
        ...
        field.position =
          initialLatex === ''
            ? 0
            : caretPoint !== null
              ? (selectionController.offsetForPoint(caretPoint) ?? 0)
              : caretPosition === null
                ? field.lastOffset
    ...
    const decorateSpecialText = () => {
      ...
      if (field.shadowRoot !== null) {
        ...
        applyLineClearance(field.shadowRoot, zoomRef.current);
      }
    };
    ...
    let pendingRawSource: string | null = null;
    ...
    applyPendingRawSourceRef.current = () => {
      const nextSource = pendingRawSource;
      pendingRawSource = null;
      if (nextSource === null || source() === nextSource) return;
      try {
        applyParsedSource(nextSource);
      } catch (error) {
