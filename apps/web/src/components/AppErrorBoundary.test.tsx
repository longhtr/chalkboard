...
describe('AppErrorBoundary', () => {
  ...
  it('renders and stores only bounded redacted recovery evidence', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const privateValue = 'private-destination@example.com';
    const token = `token=${'x'.repeat(80)}`;
    const PrivateBrokenScreen = () => {
      throw new Error(
        `render failed for ${privateValue} at https://example.com/path?${token}`,
      );
    };
    ...
    expect(
      screen.getByText(/Error: render failed for \[email\]/u),
    ).toBeInTheDocument();
    await vi.waitFor(() => {
      const stored = localStorage.getItem('chalkboard:last-recovery-error');
      expect(stored).not.toBeNull();
      const diagnostic = JSON.parse(stored ?? '{}') as Record<string, unknown>;
      expect(diagnostic).toMatchObject({
        fingerprint: expect.stringMatching(/^[0-9a-f]{64}$/u),
        fingerprintCoversCompleteValue: true,
        messageByteLength: expect.any(Number),
