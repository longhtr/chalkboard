  ...
  useEffect(() => {
    ...
    if (user !== null) {
      awareness.setLocalStateField('user', {
        color: `hsl(${[...user.id].reduce((sum, value) => sum + value.charCodeAt(0), 0) % 360} 62% 52%)`,
    ...
    const sendAwarenessUpdate = (
      ...
      const next: CloudCollaborator[] = [];
      for (const [clientId, presence] of awareness.getStates()) {
        if (clientId === awareness.clientID) continue;
        const presenceUser = presence.user as
          { color?: unknown; name?: unknown } | undefined;
        if (
