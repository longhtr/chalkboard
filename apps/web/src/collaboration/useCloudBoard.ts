...
interface CloudBoardOptions {
  ...
  /**
   * The board's access changed while it was open, so whatever this reader was
   * told their role is may no longer be true. The socket reconnects with the
   * new authority on its own; this is how the rest of the app learns to ask
   * again, instead of editing against a role the server has since revoked or
   * refusing to edit with one it has since granted.
   */
  onAccessChanged(): void;
  onRemoteBoard(elements: BoardElement[], title: string, actorId: string): void;
  /** Advances only when Workspace accepts a local semantic mutation. */
...
export function useCloudBoard({
  ...
  const handledPublicationRevisionRef = useRef(publicationRevision);
  const pendingRemoteProjectionRef = useRef<{
    current: { elements: BoardElement[]; title: string };
    stale: { elements: BoardElement[]; title: string }[];
  } | null>(null);
  ...
  const stageRemoteProjection = useCallback(
    (next: { elements: BoardElement[]; title: string }) => {
      const pending = pendingRemoteProjectionRef.current;
      const candidates =
        pending === null
          ? [publishedBoardRef.current]
          : [...pending.stale, pending.current];
      const stale = candidates
        .filter(
          (candidate) =>
            !boardContentEqual(candidate, next.elements, next.title),
        )
        .slice(-8);
      pendingRemoteProjectionRef.current = { current: next, stale };
  ...
  }, [elements, onAccessChanged, onRemoteBoard, title]);
    ...
    void loadCloudBoardCache(boardId)
      .then((cached) => {
        ...
        if (cached !== null) {
          ...
          remoteCallbackRef.current(cached.elements, cached.title, 'other');
  ...
  useEffect(() => {
    ...
    pendingRemoteProjectionRef.current = null;
    ...
    let latestRemoteActorId = 'other';
    const captureRemoteActor = (transaction: Y.Transaction) => {
      if (transaction.origin !== socket) return;
      const actorIds = new Set<string>();
      for (const [clientId, afterClock] of transaction.afterState) {
        if (afterClock <= (transaction.beforeState.get(clientId) ?? 0)) {
          continue;
        }
        const userState = awareness.getStates().get(clientId)?.user as
          { id?: unknown } | undefined;
        if (typeof userState?.id === 'string' && userState.id.length > 0) {
          actorIds.add(userState.id);
        }
      }
      latestRemoteActorId =
        actorIds.size === 1 ? ([...actorIds][0] ?? 'other') : 'other';
    };
    ...
    document.on('afterTransaction', captureRemoteActor);
    ...
    socket.addEventListener('message', (event) => {
      ...
      if (syncedRef.current) {
        const remote = readCloudBoard(document);
        stageRemoteProjection(remote);
        remoteCallbackRef.current(
          remote.elements,
          remote.title,
          latestRemoteActorId,
        );
        latestRemoteActorId = 'other';
        const pending =
          pendingUpdatesRef.current > 0 ||
          pendingRawUpdatesRef.current.length > 0 ||
          pendingCacheRef.current;
    ...
    socket.addEventListener('close', (event) => {
      if (disposed) return;
      clearConnectionAttemptTimer();
      // The gateway closes with this exact reason when a board's membership
      // changed. Other 1008 closes end the session itself and are handled by
      // the session layer, so the reason rather than the code selects this one.
      if (event.code === 1008 && event.reason === 'Board access changed') {
    ...
    return () => {
      ...
      document.off('afterTransaction', captureRemoteActor);
  ...
  useEffect(() => {
    ...
    const localPublicationRequested =
      handledPublicationRevisionRef.current !== publicationRevision;
    const pendingRemoteProjection = pendingRemoteProjectionRef.current;
    if (pendingRemoteProjection !== null) {
      if (boardContentEqual(pendingRemoteProjection.current, elements, title)) {
        pendingRemoteProjectionRef.current = null;
        publishedBoardRef.current = pendingRemoteProjection.current;
        if (localPublicationRequested) {
          handledPublicationRevisionRef.current = publicationRevision;
        }
        return;
      }
      if (!localPublicationRequested) return;
      if (
        pendingRemoteProjection.stale.some((candidate) =>
          boardContentEqual(candidate, elements, title),
        )
      ) {
        // A socket/status render ran before React applied the corresponding
        // remote board callback. Re-publishing this superseded projection would
        // turn an old local snapshot into a new Yjs edit.
        return;
      }
      // This projection is neither the remote value nor any value it
      // superseded, so it is a genuine local edit rebased over that value.
      pendingRemoteProjectionRef.current = null;
    }
    if (!localPublicationRequested) return;
  ...
  const applyHistory = useCallback(
    (direction: 'redo' | 'undo') => {
      ...
      pendingCacheRef.current = true;
      refreshPendingWork();
      stageRemoteProjection(next);
      remoteCallbackRef.current(next.elements, next.title, user?.id ?? 'local');
      cacheBoard(boardId, {
        baselineElements: baselineRef.current.elements,
        baselineTitle: baselineRef.current.title,
