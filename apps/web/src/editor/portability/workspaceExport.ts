...
interface EditableExportOptions {
  ...
  /** Local work no acknowledgement covers yet; the same signal the badge shows. */
...
>): Promise<EditableBoardExportInput> {
  ...
  // The 'saved' connection state only appears when an acknowledgement lands, so
  // an unedited board never reaches it. Durability is the question, so ask the
  // durability signal instead.
  if (cloudConnectionState !== 'read-only' && hasPendingWork) {
...
export async function exportWorkspaceImage({
  ...
  // Every format needs these; see `rasterSafe` in boardExport.
  const equationVectorMarkup = equationVectorMarkupForExport(
    themed,
    equationMarkup,
  );
