...
describe('board SVG export', () => {
  it('sizes the page around a turned shape, not around its upright box', async () => {
    // An 80x40 rectangle stood on end is 40 wide and 80 tall. Sizing the page
    // from the upright box would crop the two ends that now stick out.
    const svg = await createBoardSvg(
      input([
        {
          ...base,
          cornerRadius: 0,
    ...
    // 40 wide plus a 1px stroke halo each side, plus 24px padding each side.
    expect(Number(size?.[1])).toBeCloseTo(90, 5);
    expect(Number(size?.[2])).toBeCloseTo(130, 5);
    expect(svg).toContain('rotate(90 50 40)');
  });
