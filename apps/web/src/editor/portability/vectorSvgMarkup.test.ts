...
describe('stroke pattern markup', () => {
  it('exports a custom gap through the shared SVG dash array', () => {
    expect(
    ...
    ).toContain('stroke-dasharray:8 17');
    expect(
      vectorElementSvgMarkup({
        ...filledSquare,
        strokeDashGap: 17,
    ...
    ).toContain('stroke-dasharray:1 17');
  });
});

describe('shape fill markup', () => {
  it('completes a partial fill beneath round caps without stroking its chord', () => {
    const markup = vectorElementSvgMarkup({
      ...filledSquare,
      ellipseEndAngle: 180,
      ellipseStartAngle: 270,
      shapeKind: 'ellipse',
    });

    expect(markup).not.toContain('<ellipse');
    expect(markup).toContain('M 40,0 A 40 40 0 1 1 0,40');
    expect(markup).toContain('fill:#a5d8ff;stroke:none');
    expect(markup).toContain('L -0.707,39.293 L 39.293,-0.707 Z');
    expect(markup).toContain(
      '<path d="M 40,0 A 40 40 0 1 1 0,40" style="fill:none;stroke:',
    );

    const transparent = vectorElementSvgMarkup({
      ...filledSquare,
    ...
    expect(transparent).not.toContain(' Z');
    expect(transparent).not.toContain(' L ');
  });
...
describe('connected Straight line vector markup', () => {
  it('exports each Space-created run as an exact line', () => {
    const vertices = [
      { x: 0, y: 0 },
      { x: 100, y: 0 },
      { x: 100, y: 80 },
    ...
    const markup = vectorElementSvgMarkup(straight);
    expect(markup).toContain('M 0,0 L 100,0 L 100,80');
