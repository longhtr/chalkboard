...
/**
 * Grows a box to cover the same box after the element has been turned.
 *
 * The renderer draws a turned element by rotating the canvas around the centre
 * of its upright bounds, so a turned shape covers ground its upright box never
 * did -- a wide rectangle stood on end is taller than it was. Sizing the export
 * from the upright box alone would crop exactly that overhang.
 */
function turnedBounds(element: BoardElement, upright: Bounds): Bounds {
  if (element.rotation === 0) return upright;
  const center = elementRotationCenter(element);
  const corners = [
    { x: upright.x, y: upright.y },
    { x: upright.x + upright.width, y: upright.y },
    { x: upright.x + upright.width, y: upright.y + upright.height },
    { x: upright.x, y: upright.y + upright.height },
  ].map((corner) => rotatePoint(corner, center, element.rotation));
  return boundsForPoints(corners) ?? upright;
}

function renderedElementBounds(element: BoardElement): Bounds {
  return turnedBounds(element, uprightRenderedBounds(element));
}

function uprightRenderedBounds(element: BoardElement): Bounds {
      ...
      if (
        ...
        const direction =
          last !== undefined
            ...
            : {
                x: end.x - (finalPolylineStart?.x ?? geometry.start.x),
                y: end.y - (finalPolylineStart?.y ?? geometry.start.y),
              };
