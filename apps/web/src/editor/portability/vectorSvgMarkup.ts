...
function style(
  ...
  const pattern = strokeDashPattern(element);
  const dash = pattern.length === 0 ? 'none' : pattern.map(svgNumber).join(' ');
  return [
    `fill:${fillOverride ?? element.backgroundColor}`,
    `stroke:${strokeOverride ?? element.strokeColor}`,
...
function roundedPolylinePath(points: readonly Point[], radius: number): string {
  const corners = roundedPolylineCorners(points, radius);
  const first = corners[0];
  if (first === undefined) return '';
  let path = `M ${point(first.vertex)}`;
  for (let index = 1; index < corners.length; index += 1) {
    const corner = corners[index];
    if (corner === undefined) continue;
    path += ` L ${point(corner.entry)}`;
    if (index < corners.length - 1) {
      path += ` Q ${point(corner.vertex)} ${point(corner.exit)}`;
    }
  }
  return path;
}
...
function shapeSvg(element: BoardElement): string {
  ...
  if (kind === 'ellipse') {
    ...
    const ellipsePoint = (degrees: number): Point => {
    ...
    const start = ellipsePoint(range.startAngle);
    const end = ellipsePoint(range.endAngle);
    const largeArc = ellipseArcSweep(range) > 180 ? 1 : 0;
    const arc = `M ${point(start)} A ${svgNumber(bounds.width / 2)} ${svgNumber(bounds.height / 2)} 0 ${largeArc} 1 ${point(end)}`;
    const fillStyle =
      element.type === 'shape' ? (element.fillStyle ?? 'solid') : 'solid';
    let fill = '';
    if (element.backgroundColor !== 'transparent' && fillStyle === 'solid') {
      ...
      if (shiftedEnd !== undefined && shiftedStart !== undefined) {
        const fillPath = `${arc} L ${point(shiftedEnd)} L ${point(shiftedStart)} Z`;
        fill = `<path d="${fillPath}" style="${style(element, undefined, 'none')}"/>`;
      }
    }
    const stroke = `<path d="${arc}" style="${style(element, 'none')}"/>`;
    return `${fill}${hatch}${stroke}`;
  }
...
function linearSvg(element: BoardElement): string {
  ...
  const orthogonalRadius =
    element.type === 'line' && element.pathKind === 'orthogonal'
      ? (element.cornerRadius ?? 0)
      : 0;
  let pathData = `M ${point(geometry.start)}`;
  if (orthogonalRadius > 0) {
    // Export must trace the same rounded outline the canvas draws, not the
    // sharp cubics the element stores.
    pathData = roundedPolylinePath(linePathVertices(element), orthogonalRadius);
  } else if (geometry.kind === 'straight') {
    pathData += ` L ${point(geometry.end)}`;
  } else if (geometry.kind === 'polyline') {
    for (const pathPoint of geometry.points.slice(1)) {
      pathData += ` L ${point(pathPoint)}`;
    }
  } else {
  ...
  const end =
    geometry.kind === 'straight'
      ...
      : geometry.kind === 'polyline'
        ...
        : last?.end;
  const firstPolylineEnd =
    geometry.kind === 'polyline' ? geometry.points[1] : undefined;
  const finalPolylineStart =
    geometry.kind === 'polyline' ? geometry.points.at(-2) : undefined;
    ...
    const direction =
      ...
      Math.hypot(controlDirection.x, controlDirection.y) > 1e-6
        ...
        : {
            x:
              geometry.start.x -
              (firstPolylineEnd?.x ?? end?.x ?? geometry.start.x),
            y:
              geometry.start.y -
              (firstPolylineEnd?.y ?? end?.y ?? geometry.start.y),
          };
    ...
    const direction =
      ...
      Math.hypot(controlDirection.x, controlDirection.y) > 1e-6
        ...
        : {
            x:
              last === undefined
                ? end.x - (finalPolylineStart?.x ?? geometry.start.x)
                : last.end.x - last.start.x,
            y:
              last === undefined
                ? end.y - (finalPolylineStart?.y ?? geometry.start.y)
...
export function vectorElementSvgMarkup(element: BoardElement): string {
  const markup = unrotatedElementSvgMarkup(element);
  if (markup === '' || element.rotation === 0) return markup;
  // The canvas draws a rotated element by turning the canvas around the centre
  // of its unrotated bounds. An SVG group doing the same thing keeps an export
  // identical to what was on screen; rotating the stored coordinates instead
  // would have to round every point and would drift from the canvas.
  const center = elementRotationCenter(element);
  const transform = `rotate(${svgNumber(element.rotation)} ${svgNumber(center.x)} ${svgNumber(center.y)})`;
  return `<g transform="${transform}">${markup}</g>`;
}

function unrotatedElementSvgMarkup(element: BoardElement): string {
