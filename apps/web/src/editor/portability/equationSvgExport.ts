...
import { mathSegments, parseMixedText } from '../../math/mixedMath';
import { escapeXml } from './vectorSvgMarkup';

function mathOnlySource(source: string): boolean {
  const math = mathSegments(source);
  return (
    math.length === 1 &&
    parseMixedText(source).every(
...
export function equationMarkupForExport(
  ...
  for (const element of elements) {
    ...
    container.innerHTML = staticMathMarkup(element.source, {
      ...
      isMathOnly: mathOnlySource(element.source),
...
function textMarkup(node: Node, rootBounds: DOMRect): string {
  ...
  const literal = parent.classList.contains('mixed-text-literal-dollar')
    ? '$'
    : parent.classList.contains('mixed-text-literal-backslash')
      ? '\\'
      : null;
  ...
  return `<text x="${x}" y="${y}" fill="${escapeXml(style.color)}" font-family="${escapeXml(style.fontFamily)}" font-size="${escapeXml(style.fontSize)}" font-style="${escapeXml(style.fontStyle)}" font-weight="${escapeXml(style.fontWeight)}" dominant-baseline="text-before-edge" textLength="${bounds.width}" lengthAdjust="spacingAndGlyphs" xml:space="preserve">${escapeXml(value)}</text>`;
}
...
function createStagingElement(
  ...
  content.className = mathOnlySource(element.source)
