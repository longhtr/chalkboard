...
function drawBezierHandles(
  ...
  if (element.pathKind !== 'bezier' && element.pathKind !== 'orthogonal')
    return;
  const path = linePathGeometry(element);
  ...
  // Orthogonal corners are draggable but have no editable cubic controls, so
  // the control dots and their dashed tethers stay hidden for that path kind.
