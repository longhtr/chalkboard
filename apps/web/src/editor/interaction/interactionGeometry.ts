...
/** Finds the nearest visible Bézier node/control for a single selected line. */
export function findBezierHandle(
  ...
  if (line?.type !== 'line' || line.pathKind !== 'bezier') return null;
  ...
  if (!hasLockedPeriodicC2Controls(line)) {
    path.segments.forEach((segment, segmentIndex) => {
      handles.push(
        [
