...
describe('drawing interaction controller', () => {
  ...
  it('continues Straight segments and snaps only to earlier segment starts', () => {
    ...
    expect(addStraightDrawingPoint(current)).toBe(true);
    updateDrawingInteraction(current, {
      constrain: false,
      samples: [],
      worldPoint: { x: 100, y: 80 },
      zoom: 1,
    });
    expect(addStraightDrawingPoint(current)).toBe(true);

    const snapped = updateDrawingInteraction(current, {
      constrain: false,
      samples: [],
      worldPoint: { x: 8, y: 5 },
      zoom: 1,
    }) as LineElement;
    expect(snapped.segments.map(({ end }) => end)).toEqual([
      { x: 100, y: 0 },
      { x: 100, y: 80 },
      { x: 0, y: 0 },
    ]);
    expect(
      finalizeDrawingInteraction(current, {
        fit: { accuracy: 1, continuity: 'c1', maxSegments: 8 },
        point: { x: 8, y: 5 },
        zoom: 1,
      }),
    ).toMatchObject({ height: 0, width: 0 });
