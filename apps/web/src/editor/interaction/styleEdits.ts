...
/** Applies an arc range only to targeted ellipse shapes. */
export function updateEllipseArc(
  elements: readonly BoardElement[],
  targetIds: ReadonlySet<string>,
  change: Pick<ShapeElement, 'ellipseEndAngle' | 'ellipseStartAngle'>,
): BoardElement[] | null {
  let changed = false;
  const updated = elements.map((element) => {
    if (
...
export function updateLinePathKind(
  ...
  const updated = elements.map((element) => {
    ...
    changed = true;
    const {
      splineContinuity: removedContinuity,
      straightSegmented: removedStraightMarker,
      ...line
    } = element;
    void removedContinuity;
    void removedStraightMarker;
    if (pathKind === 'bezier') {
      return enforceBezierContinuity({ ...line, pathKind }, continuity);
    }
    if (pathKind === 'straight') {
      ...
      return {
        ...line,
        pathKind,
        segments: polylineCubicSegments([start, end], start),
      };
    }
