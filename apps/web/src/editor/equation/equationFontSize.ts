...
export function focusEquationEditorAfterControl(
  ...
  return window.requestAnimationFrame(() => {
    ...
    const editor = document.querySelector<HTMLElement>(
      view === 'source' ? 'textarea[aria-label="Block source"]' : 'math-field',
    );
    // Handing focus back after a toolbar control is the whole point of this
    // call, so it goes through even though a control still holds focus.
    if (editor !== null) focusDeliberately(editor, { preventScroll: true });
  });
}
