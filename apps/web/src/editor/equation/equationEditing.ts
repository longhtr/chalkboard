...
export interface EditingEquation {
  ...
  isNew: boolean;
  /** Latest collaborative base for the still-pending local source delta. */
  localEditBaseSource?: string;
  /** Local source delta rebased over `localEditBaseSource`. */
  localEditSource?: string;
  sessionId: string;
  source: string;
  width: number;
}
