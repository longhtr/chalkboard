...
import { PublicInformationDialog } from '../public/PublicInformationPage';
import type { PublicInformationPath } from '../public/publicRoutes';
import {
...
export function Workspace({
  ...
  const toggleObjectNavigator = useCallback(() => {
    ...
    if (!objectNavigatorOpen) setObjectNavigatorFocusOnOpen(true);
  ...
  const [informationPath, setInformationPath] =
    useState<PublicInformationPath | null>(null);
  const [informationOptionsOpen, setInformationOptionsOpen] = useState(false);
  ...
  useKeyboardCommands({
    ...
    modalOpen: modalOpen || informationPath !== null,
    readOnly,
    strokeColors,
    toolOrder: supportedToolOrder,
  ...
  return (
    <main className={readOnly ? 'workspace is-read-only' : 'workspace'}>
      <BoardMenu
        ...
        onOpenPublicInformation={(path) => {
          // Keep both menu levels mounted behind the modal so dismissing one
          // information page immediately restores the destination chooser.
          setObjectNavigatorOpen(false);
          setInformationPath(path);
        }}
        onOpenShortcuts={() => {
          setMenuOpen(false);
          setObjectNavigatorOpen(false);
        ...
        onToggleFontSettings={() => {
          setNewBoardOptionsOpen(false);
          setInformationOptionsOpen(false);
        ...
        onToggleInformationOptions={() => {
          setNewBoardOptionsOpen(false);
          setGridSettingsOpen(false);
          setFontSettingsOpen(false);
          setThemeSettingsOpen(false);
          setInformationOptionsOpen((current) => !current);
        }}
        onToggleMenu={() => {
          if (!menuOpen) {
            setNewBoardOptionsOpen(false);
      ...
      {informationPath === null ? null : (
        <PublicInformationDialog
          pathname={informationPath}
          onClose={() => setInformationPath(null)}
        />
      )}
