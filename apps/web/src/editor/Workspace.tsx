...
const DEFAULT_STROKE_COLORS = ['#1f2937', '#e03131', '#1971c2'];
const DEFAULT_FILL_COLORS = ['#e9ecef', '#ffc9c9', '#a5d8ff'];
...
export function Workspace({
  ...
  // The reducer owns committed history and sparse previews. Persistence and
  // collaboration consume only its committed `present` document.
  ...
  const [gridSpacing, setGridSpacing] = useState(loadGridSpacing);
  const [gridDotSize, setGridDotSize] = useState(loadGridDotSize);
  ...
  const [strokeColors, setStrokeColors] = useState(() => [
    ...
    ...loadCustomColors(LOCAL_CUSTOM_COLORS_KEY, DEFAULT_STROKE_COLORS),
  ]);
  const [fillColors, setFillColors] = useState(() => [
    'transparent',
    ...DEFAULT_FILL_COLORS,
    ...loadCustomColors(LOCAL_CUSTOM_FILL_COLORS_KEY, DEFAULT_FILL_COLORS),
  ]);
  ...
  const handleRemoteCloudBoard = useCallback(
    (elements: BoardElement[], title: string) => {
      ...
      setEditingEquation((current) => {
        ...
        return remote.source === current.source &&
          remote.width === current.width &&
          remote.height === current.height
          ? current
          : {
              ...current,
    ...
    [handleExternalBoard, setEditingEquation],
  );
  ...
  const {
    canRedo: canRedoCloud,
    canUndo: canUndoCloud,
  ...
  // History follows the active durability model. Local reducer history and Yjs
  // undo must never run for the same gesture.
  const applyDocumentHistory = useCallback(
    (direction: 'redo' | 'undo') => {
      if (cloudBoard === null) {
        dispatchDocument({ type: direction });
    ...
    [cloudBoard, redoCloud, undoCloud],
  );
  ...
  const contentLayers = useMemo(
    () => groupContentLayers(nearbyDisplayElements),
    [nearbyDisplayElements],
  );
  ...
  const displayedCornerRadius = selectedShape?.cornerRadius ?? cornerRadius;
  ...
  const showInputModeControls = usesTextColor && activeTool === 'equation';
  ...
  const displayedTextSize = Math.round(
    selectedEquationForStyle?.fontSize ??
  ...
  usePreferencePersistence({
    bezierFit,
    fillColors,
    firstCustomFillColor: DEFAULT_FILL_COLORS.length + 1,
    firstCustomStrokeColor: DEFAULT_STROKE_COLORS.length,
    gridDotSize,
    gridSpacing,
    inputMode,
    lineSpacing,
  ...
  const commitElements = useCallback(
    (elements: BoardElement[]) => {
      ...
      dispatchDocument({ type: 'commit', elements });
      return true;
    },
    [dispatchDocument, rejectBoardElementLimit],
  );
  ...
  const {
    ...
    editingEquation,
    elementStyle,
    equationSourceView: editingView === 'source',
    elements: documentState.present,
  ...
  useKeyboardCommands({
    ...
    currentStrokeColor: elementStyle.strokeColor,
    ...
    requestHistory: (direction) => {
      ...
      if (field !== null) {
        ...
        applyDocumentHistory(direction);
      }
    },
    selectTool,
    setTypingColor: (strokeColor) => {
      const next = { ...elementStyle, strokeColor };
    ...
    toggleSelectionObjects: () => {
      setObjectNavigatorOpen((current) => !current);
    },
  });
  ...
  }, [overlayCanvasRef, panBy, zoomByFactor]);
  ...
  const updateSelectedStyle = (change: Partial<ElementStyle>) => {
    ...
    if (usesTextColor && change.strokeColor !== undefined) {
      if (editingEquation !== null) {
        requestEquationTypingColor(change.strokeColor);
      }
      return;
    }
  ...
  const updateSelectedShape = (
    change: Partial<Pick<ShapeElement, 'cornerRadius' | 'shapeKind'>>,
  ) => {
  ...
  const removeCustomColor = (color: string) => {
    if (DEFAULT_STROKE_COLORS.includes(color)) return;
    setStrokeColors((current) => current.filter((value) => value !== color));
    if (elementStyle.strokeColor === color) {
      updateSelectedStyle({
        strokeColor: DEFAULT_STROKE_COLORS[0] ?? '#1f2937',
      });
    }
  };

  const removeCustomFillColor = (color: string) => {
    if (DEFAULT_FILL_COLORS.includes(color)) return;
    setFillColors((current) => current.filter((value) => value !== color));
    if (elementStyle.backgroundColor === color) {
      updateSelectedStyle({ backgroundColor: 'transparent' });
    }
  };
    ...
    <main className={readOnly ? 'workspace is-read-only' : 'workspace'}>
      ...
      >
        ...
        {boardLimitStatus !== null && (
          <div className="operation-status is-error" role="alert">
            <span>{boardLimitStatus}</span>
            <button
              className="operation-status__action"
              type="button"
              onClick={() => setBoardLimitStatus(null)}
            >
              Dismiss
            </button>
          </div>
          ...
          >
            ...
            {operationStatus.cancel !== undefined && (
              <button
                className="operation-status__action"
                type="button"
                onClick={operationStatus.cancel}
              >
                Cancel
              </button>
            )}
      ...
      {objectNavigatorOpen ? (
        <ObjectNavigator
          ...
          onCenterObject={(id) => {
            ...
            centerAtVerticalStart(elementBounds(element));
          }}
          onClose={closeObjectNavigator}
        ...
        >
          {showColorControls ? (
            <>
              ...
              <ColorSwatches
                activeColor={displayedStyle.strokeColor}
                colors={
                  activeTool === 'shape' || selectedShape !== undefined
                    ? ['transparent', ...strokeColors]
                    : strokeColors
                }
                defaultColors={DEFAULT_STROKE_COLORS}
                kind={usesTextColor ? 'text' : 'stroke'}
                onChange={(strokeColor) => updateSelectedStyle({ strokeColor })}
          ...
          {activeTool === 'shape' && (
            <ShapeControls
              onChange={(shapeKind) => updateSelectedShape({ shapeKind })}
          ...
          {(activeTool === 'shape' ||
            ...
            <StrokeControls
              ...
              onCornerRadiusChange={(cornerRadius) =>
                updateSelectedShape({ cornerRadius })
              }
          ...
          {usesTextColor && (
            <TextControls
              ...
              onEditingViewChange={setEditingView}
              ...
              showEditingView={editingEquation !== null}
          ...
          {(activeTool === 'shape' || selectedShape !== undefined) && (
            <>
              ...
              <ColorSwatches
                activeColor={displayedStyle.backgroundColor}
                colors={fillColors}
                defaultColors={DEFAULT_FILL_COLORS}
                kind="fill"
                onChange={(backgroundColor) =>
                  updateSelectedStyle({ backgroundColor })
                }
              ...
              />
              {displayedStyle.backgroundColor !== 'transparent' && (
                <>
                    ...
                    {(
                      ...
                      >
                        <span
                          ...
                          style={{ color: displayedStyle.backgroundColor }}
                        />
                      </button>
                    ))}
                  </div>
        ...
        <div className="dom-layer">
          ...
          {editingEquation !== null && editorElement !== undefined ? (
            <EquationEditor
              ...
              typingColor={elementStyle.strokeColor}
            />
          ) : null}
        </div>
        <canvas
      ...
      {!readOnly ? (
        <HistoryControls
          canRedo={
            cloudBoard === null ? documentState.future.length > 0 : canRedoCloud
          }
          canUndo={
            cloudBoard === null ? documentState.past.length > 0 : canUndoCloud
          }
          onRedo={() => applyDocumentHistory('redo')}
          onUndo={() => applyDocumentHistory('undo')}
        />
      ) : null}
