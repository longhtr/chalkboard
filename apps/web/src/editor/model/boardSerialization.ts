...
export function isStoredBoardElement(value: unknown): value is BoardElement {
  ...
  const baseIsValid =
    ...
    (element.strokeDashGap === undefined ||
      (typeof element.strokeDashGap === 'number' &&
        Number.isFinite(element.strokeDashGap) &&
        element.strokeDashGap >= MIN_STROKE_DASH_GAP));
  ...
  if (type === 'line') {
    return (
      ...
      (element.straightSegmented === undefined ||
        (element.pathKind === 'straight' &&
          element.straightSegmented === true)) &&
      // Bounded below but not above, matching the shape radius: rendering
      // clamps to half the shorter run, so a wider stored value still draws.
  ...
  if (type === 'shape') {
    return (
      ...
      Number.isFinite(element.cornerRadius) &&
      element.cornerRadius >= 0 &&
      (element.ellipseStartAngle === undefined ||
        (typeof element.ellipseStartAngle === 'number' &&
          Number.isFinite(element.ellipseStartAngle) &&
          element.ellipseStartAngle >= MIN_ELLIPSE_ANGLE &&
          element.ellipseStartAngle <= MAX_ELLIPSE_ANGLE)) &&
      (element.ellipseEndAngle === undefined ||
        (typeof element.ellipseEndAngle === 'number' &&
          Number.isFinite(element.ellipseEndAngle) &&
          element.ellipseEndAngle >= MIN_ELLIPSE_ANGLE &&
          element.ellipseEndAngle <= MAX_ELLIPSE_ANGLE)) &&
      Number(element.ellipseStartAngle ?? DEFAULT_ELLIPSE_START_ANGLE) !==
        Number(element.ellipseEndAngle ?? DEFAULT_ELLIPSE_END_ANGLE) &&
...
function migrateStoredLine(element: LineElement): LineElement {
  ...
  // Only orthogonal turns round, so a radius stored against any other kind is
  // dropped rather than carried forward as a field nothing reads.
  if (stored.pathKind !== 'orthogonal') delete normalizedStored.cornerRadius;
