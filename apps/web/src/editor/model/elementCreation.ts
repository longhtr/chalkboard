...
export function createElement(
  ...
  if (type === 'equation') {
    return {
      ...
      lineSpacing,
      source: '',
      sourceFontSize: textSize - EQUATION_SOURCE_FONT_SIZE_OFFSET,
      type,
      width: 32,
    };
  }
  ...
  return { ...base, cornerRadius, fillSpacing, fillStyle, shapeKind, type };
}
...
export function finalizeDrawing(element: BoardElement): BoardElement | null {
  ...
  if (element.type === 'line') {
    if (element.pathKind === 'straight') {
      return Math.hypot(element.width, element.height) < 3 ? null : element;
    }
