...
describe('board serialization', () => {
  ...
  it('round-trips valid theme colors and rejects malformed optional values', () => {
    const themed = {
      ...baseElement,
      backgroundColorDark: '#5c3130',
      strokeColorDark: '#ff8787',
      type: 'rectangle',
    };

    expect(parseStoredElements(JSON.stringify([themed]))[0]).toMatchObject({
      backgroundColorDark: '#5c3130',
      strokeColorDark: '#ff8787',
    });
    expect(isStoredBoardElement({ ...themed, backgroundColorDark: 123 })).toBe(
      false,
    );
    expect(isStoredBoardElement({ ...themed, strokeColorDark: false })).toBe(
      false,
    );
  });
  ...
  it('bounds stroke gaps below without dropping wider boards', () => {
    const dashed = {
      ...baseElement,
      strokeStyle: 'dashed',
      type: 'rectangle',
    ...
    for (const strokeDashGap of [1, 8, 100, 4_000]) {
      expect(isStoredBoardElement({ ...dashed, strokeDashGap })).toBe(true);
    }
    expect(isStoredBoardElement(dashed)).toBe(true);
    for (const strokeDashGap of [0, -1, Number.NaN, '8']) {
      expect(isStoredBoardElement({ ...dashed, strokeDashGap })).toBe(false);
    }
  });

  it('accepts bounded non-empty ellipse arcs that wrap through zero', () => {
    const ellipse = {
      ...baseElement,
      cornerRadius: 0,
      shapeKind: 'ellipse',
    ...
    expect(
      parseStoredElements(
        JSON.stringify([
          { ...ellipse, ellipseEndAngle: 180, ellipseStartAngle: 0 },
        ]),
      )[0],
    ).toMatchObject({ ellipseEndAngle: 180, ellipseStartAngle: 0 });
    expect(isStoredBoardElement(ellipse)).toBe(true);
    expect(
      isStoredBoardElement({
        ...ellipse,
        ellipseEndAngle: 60,
    ...
    expect(
      isStoredBoardElement({
        ...ellipse,
        ellipseEndAngle: 0,
        ellipseStartAngle: 360,
      }),
    ).toBe(true);
    for (const range of [
      { ellipseEndAngle: 0, ellipseStartAngle: 0 },
      { ellipseEndAngle: 180, ellipseStartAngle: 180 },
      { ellipseEndAngle: 361, ellipseStartAngle: 0 },
      { ellipseEndAngle: 180, ellipseStartAngle: -1 },
    ]) {
      expect(isStoredBoardElement({ ...ellipse, ...range })).toBe(false);
    }
  });
  ...
  it('preserves the expanded text size and line-spacing limits', () => {
    const maximum = {
      ...baseElement,
      fontSize: 200,
      lineSpacing: 5,
    ...
    expect(parseStoredElements(JSON.stringify([maximum]))[0]).toMatchObject({
      fontSize: 200,
      lineSpacing: 5,
      sourceFontSize: 200,
    });
    expect(
      parseStoredElements(
        JSON.stringify([{ ...maximum, lineSpacing: 5.1 }]),
      )[0],
    ).toMatchObject({ lineSpacing: 1.2 });
  });
  ...
  it('round-trips intentional connected Straight paths only with their marker', () => {
    const connected = {
      ...baseElement,
      height: 60,
      pathKind: 'straight',
    ...
    expect(parseStoredElements(JSON.stringify([connected]))).toEqual([
      { ...connected, arrowheads: 'none', strokeStyle: 'solid' },
    ]);
    expect(
      parseStoredElements(
        JSON.stringify([{ ...connected, pathKind: 'bezier' }]),
      ),
    ).toEqual([]);
  });
