...
/**
 * What the panel can tell a reader about the order they are looking at.
 *
 * Both the hover hint and the screen-reader description say this, so a sighted
 * reader waiting over a row and a reader hearing the list are told the same
 * thing. Only the layer order can be dragged, so only it mentions dragging;
 * saying so under the vertical order would describe a gesture that does
 * nothing there.
 */
function orderHint(sortMode: ObjectSortMode): string {
  return sortMode === 'layer'
    ? 'Shift-click to select multiple objects. Drag a selected object to move the selection.'
  ...
  return (
    ...
    >
      ...
      <span className="sr-only" id="object-navigator-order-hint">
        {sortMode === 'layer'
          ? `${orderHint(sortMode)} Use the Layer order buttons as a keyboard alternative.`
          : orderHint(sortMode)}
      </span>
