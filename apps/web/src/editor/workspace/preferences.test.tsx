...
describe('workspace preferences', () => {
  it('loads defaults for missing or malformed values', () => {
    ...
    localStorage.setItem('chalkboard:source-text-size', '1000');
    ...
    expect(loadSourceTextSize(DEFAULT_TEXT_SIZE)).toBe(DEFAULT_TEXT_SIZE - 3);
  ...
  it('normalizes valid stored settings', () => {
    ...
    localStorage.setItem('chalkboard:source-text-size', '36');
    ...
    expect(loadSourceTextSize(48)).toBe(36);
  ...
  it('accepts the full text-size and line-spacing limits', () => {
    localStorage.setItem('chalkboard:text-size', '200');
    localStorage.setItem('chalkboard:source-text-size', '200');
    localStorage.setItem('chalkboard:line-spacing', '5');

    expect(loadTextSize()).toBe(200);
    expect(loadSourceTextSize()).toBe(200);
    expect(loadLineSpacing()).toBe(5);

    localStorage.setItem('chalkboard:text-size', '201');
    localStorage.setItem('chalkboard:source-text-size', '201');
    localStorage.setItem('chalkboard:line-spacing', '5.1');
    expect(loadTextSize()).toBe(DEFAULT_TEXT_SIZE);
    expect(loadSourceTextSize(DEFAULT_TEXT_SIZE)).toBe(DEFAULT_TEXT_SIZE - 3);
    expect(loadLineSpacing()).toBe(DEFAULT_LINE_SPACING);
  });
  ...
  it('persists bounded workspace preference values through one owner', () => {
    ...
    expect(localStorage.getItem('chalkboard:source-text-size')).toBe('39');
