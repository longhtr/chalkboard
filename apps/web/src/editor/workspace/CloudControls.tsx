...
interface CloudControlsProps {
  ...
  /**
   * Membership role, not transport state. Taken separately so the role stays on
   * screen while the connection changes underneath it.
   */
  viewerRole?: boolean;
}
...
export function CloudControls({
  ...
  // One avatar per account, not per tab. Cursors and selections stay keyed by
  // client id in CollaborationOverlay, because two tabs are genuinely two
  // pointers in two places.
  const presentCollaborators = useMemo(
    () => distinctCollaborators(collaborators),
    [collaborators],
  );
  const outstanding =
    draftPending ||
  ...
  // A local board has no transport at all, so its pill stands alone. A viewer
  // does have one, and needs it: nothing they do would reveal a dead socket, so
  // the board would just quietly stop changing. Their role therefore sits
  // beside the transport rather than instead of it, which also stops the pill
  // flipping to a bare "Connected" the moment the socket is in trouble.
  const localOnly = connectionState === 'local';
  ...
  return (
    <div className="workspace-top-right">
      {cloudBoardActive && (
        <>
          {localOnly ? (
            <span className="cloud-status is-local" role="status">
            ...
            <>
              {connectionState === 'connection-failed' ? (
                <button
                  // Same state as the disconnected badge, so it carries the
                  // same colour. `.cloud-retry` alone is the warning palette,
                  // which left one "Disconnected" red and the other yellow.
                  className="cloud-status cloud-retry is-disconnected"
                  type="button"
                  onClick={onRetryConnection}
                >
                  <span
                    className="cloud-status-dot is-disconnected"
                    aria-hidden="true"
                  />
                  Disconnected — Retry
                </button>
              ...
              {viewerRole && (
                <span className="cloud-status is-read-only" role="status">
      ...
      {presentCollaborators.slice(0, 3).map((collaborator) => (
        <span
          className="avatar collaborator-avatar"
          key={collaborator.userId ?? `client:${collaborator.clientId}`}
