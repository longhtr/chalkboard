...
const LOCAL_SOURCE_TEXT_SIZE_KEY = 'chalkboard:source-text-size';
...
/** Reads the source view's independent default text size. */
export function loadSourceTextSize(renderedTextSize = loadTextSize()): number {
  const value = Number(
    bestEffortLocalStorage.getItem(LOCAL_SOURCE_TEXT_SIZE_KEY),
  );
  return Number.isInteger(value) &&
    value >= MIN_TEXT_SIZE &&
    value <= MAX_TEXT_SIZE
    ? value
    : Math.max(1, renderedTextSize - EQUATION_SOURCE_FONT_SIZE_OFFSET);
}
...
export function usePreferencePersistence({
  ...
  useEffect(() => {
    bestEffortLocalStorage.setItem(
      LOCAL_SOURCE_TEXT_SIZE_KEY,
      String(sourceTextSize),
    );
  }, [sourceTextSize]);
  useEffect(() => {
