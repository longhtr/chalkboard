...
describe('usePointerController', () => {
  ...
  it('initializes independent rendered and source font sizes from the active view', () => {
    ...
    expect(rendered.onBeginEquationEdit).toHaveBeenCalledWith(
      expect.objectContaining({ fontSize: 25, sourceFontSize: 20 }),
      { caretPoint: { x: 20, y: 30 }, isNew: true },
    );

    const source = { ...options('equation'), equationSourceView: true };
    const sourceController = renderHook(() => usePointerController(source));
    act(() =>
      sourceController.result.current.onPointerDown(
        pointerEvent('down', 40, 50),
      ),
    );
    expect(source.onBeginEquationEdit).toHaveBeenCalledWith(
      expect.objectContaining({ fontSize: 30, sourceFontSize: 25 }),
      { caretPoint: { x: 40, y: 50 }, isNew: true },
    );
  });
