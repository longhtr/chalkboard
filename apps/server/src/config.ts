...
const environmentSchema = z.object({
  ...
  RESEND_API_KEY_FILE: z
    .string()
    .trim()
    .min(1)
    .default('/run/secrets/resend-api-key'),
  RESEND_WEBHOOK_SECRET_FILE: z
    .string()
    .trim()
    .min(1)
    .default('/run/secrets/resend-webhook-secret'),
...
export interface AppConfig {
  ...
  /** Resend delivery configuration. Both secrets are read from mounted files. */
  resendEmail: {
    apiKeyFile: string;
    from: string;
    replyTo: string;
...
export function loadConfig(
  ...
  // Unknown keys are stripped rather than rejected, so a stale provider
  // setting would otherwise linger in a deployed environment file and quietly
  // suggest a transport that no longer exists. Refuse them by name instead.
  ...
  if (retiredSettings.length > 0) {
    throw new Error(
      `Invalid environment configuration: ${retiredSettings.join(', ')} ${
        retiredSettings.length === 1 ? 'is' : 'are'
      } no longer used and must be removed`,
    );
  }

  const senderFields = [result.data.EMAIL_FROM, result.data.EMAIL_REPLY_TO];
  if (
    !senderFields.every((value) => value === undefined) &&
    !senderFields.every((value) => value !== undefined)
  ) {
    throw new Error(
      'Invalid environment configuration: EMAIL_FROM and EMAIL_REPLY_TO must be provided together',
    );
  }
  if (result.data.NODE_ENV === 'production' && senderFields[0] === undefined) {
    throw new Error(
  ...
  return {
    ...
    resendEmail:
      ...
      result.data.EMAIL_REPLY_TO === undefined
        ? null
        : {
            apiKeyFile: result.data.RESEND_API_KEY_FILE,
            from: result.data.EMAIL_FROM,
            replyTo: result.data.EMAIL_REPLY_TO,
            webhookSecretFile: result.data.RESEND_WEBHOOK_SECRET_FILE,
          },
  };
}
