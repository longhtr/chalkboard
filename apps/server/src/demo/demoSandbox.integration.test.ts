/**
 * Demo sandbox against a real database: reset health, the demo/normal
 * partition, and the quotas that keep shared demo content bounded.
 *
 * These need PostgreSQL because the rules they check are enforced by triggers
 * and constraints, not by application code, and because several of them are
 * about what happens when two writers race.
 */
...
describe.skipIf(connectionString === undefined)('writable demo sandbox', () => {
  ...
  it('derives immutable partitions and rejects every mixed membership path', async () => {
    ...
    // Sharing offers an invitation now; the demo boundary is what is under
    // test here, so it is the offer that must respect it.
    expect(
      await boards.inviteMember(
        firstDemo.id,
        demoBoard.id,
