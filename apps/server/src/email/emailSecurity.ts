...
const FLOW_LIMITS: Record<EmailFlow, readonly Limit[]> = {
  registration: [
    { count: 3, milliseconds: HOUR_MS, scope: 'ip' },
    { count: 10, milliseconds: DAY_MS, scope: 'ip' },
...
export function createEmailSecurityService(
  ...
  const dailyCapacity = options.capacityLimits?.daily ?? 80;
  const monthlyCapacity = options.capacityLimits?.monthly ?? 2_400;
