...
export interface ResendMaterial {
  apiKey: string;
...
/**
 * Reads the provider credentials. These are kept in a separate failure domain
 * from admission and Turnstile material: a missing provider credential must
 * disable email delivery alone, never human verification or board admission.
 */
export function loadResendMaterial(
  configuration: AppConfig['resendEmail'],
  onFailure: (error: unknown) => void = () => undefined,
): ResendMaterial | null {
  if (configuration === null) return null;
  try {
    const apiKey = readBounded(configuration.apiKeyFile);
    const webhookSecret = readBounded(configuration.webhookSecretFile);
    if (apiKey.length < 8 || webhookSecret.length < 8) {
      try {
        onFailure(new Error('Resend material failed validation'));
      } catch {
        // Failure reporting must not change fail-closed startup behavior.
      }
      return null;
