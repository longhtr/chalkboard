...
async function deliver(input: {
  ...
  const sendAndRecord = async (): Promise<'accepted' | 'unavailable'> => {
    ...
    try {
      ...
      // An ambiguous result may already have accepted the message, and the
      // adapter has already made its one idempotent retry. Keep the pending
      // code and reservation, and never create another intent for it.
      return 'accepted';
    }
