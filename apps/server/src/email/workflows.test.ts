...
function accounts(overrides: Partial<AccountService> = {}): AccountService {
  return {
    ...
    boardInvitationPreference: vi.fn(async () => true),
    setBoardInvitationPreference: vi.fn(async () => true),
...
describe('account email workflows', () => {
  ...
  it('retains provider acceptance correlation when accepted-send bookkeeping fails', async () => {
  ...
  // A suppressed destination is a permanent policy decision. Collapsing it into
  // the transient outcome told the reader to reconnect and retry, which can
  // never succeed.
  it.each([
    ['suppressed', 'suppressed-destination'],
    ['disabled', 'unavailable'],
  ] as const)(
    'reports a %s email-change admission as %s',
  ...
  it('reports a suppressed reservation apart from exhausted capacity', async () => {
  ...
  // Sending a verification code to the address the caller is already signed in
  // with proves nothing and spends one of the day's provider sends.
  it('refuses a change to the address the account already holds', async () => {
    const emailSender = sender();
    const workflow = createAccountEmailWorkflows({
      accounts: accounts({
        beginEmailChange: vi.fn(async () => ({
  ...
  it('keeps an unauthenticated registration generic for a suppressed address', async () => {
    const emailSender = sender();
    const workflow = createAccountEmailWorkflows({
      accounts: accounts(),
      addressValidator: validAddress,
