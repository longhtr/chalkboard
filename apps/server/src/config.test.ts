...
const productionEmail = {
  ...
  SES_FEEDBACK_TOPIC_ARN: 'arn:aws:sns:us-east-1:000000000000:feedback',
...
describe('loadConfig', () => {
  ...
  it('configures lower canary email capacity without exceeding hard caps', () => {
    ...
    expect(() => loadConfig({ EMAIL_DAILY_SEND_LIMIT: '81' })).toThrow(
      'Invalid environment configuration',
    );
    expect(() => loadConfig({ EMAIL_MONTHLY_SEND_LIMIT: '2401' })).toThrow(
      'Invalid environment configuration',
    );
    expect(() =>
      loadConfig({
  ...
  it('requires complete email delivery configuration in production', () => {
    expect(loadConfig({}).verificationEmail).toBeNull();
    expect(
      loadConfig({
        ...
        EMAIL_REPLY_TO: 'support@example.com',
        PUBLIC_ORIGIN: 'https://chalkboard.example',
        SES_FEEDBACK_TOPIC_ARN: 'arn:aws:sns:eu-west-1:000000000000:feedback',
      }).verificationEmail,
    ).toEqual({
      configurationSet: 'chalkboard-transactional',
      feedbackTopicArn: 'arn:aws:sns:eu-west-1:000000000000:feedback',
  ...
  it('bounds SNS ownership confirmation to an explicit exact-region setting', () => {
    expect(loadConfig({ SNS_CONFIRM_SUBSCRIPTION: 'true' })).toMatchObject({
      snsConfirmSubscription: true,
    });
    expect(() => loadConfig({ SNS_CONFIRM_SUBSCRIPTION: 'yes' })).toThrow(
      'Invalid environment configuration',
    );
    expect(() =>
      loadConfig({
        AWS_REGION: 'us-east-1',
        EMAIL_CONFIGURATION_SET: 'transactional',
        EMAIL_FROM: 'Chalkboard <accounts@example.com>',
        EMAIL_REPLY_TO: 'support@example.com',
        SES_FEEDBACK_TOPIC_ARN: 'arn:aws:sns:eu-west-1:000000000000:feedback',
      }),
    ).toThrow('SES feedback topic and AWS region must match');
  });
