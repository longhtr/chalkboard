...
export interface AccountService {
  ...
  /** Whether other people may offer this account access to their boards. */
  boardInvitationPreference(userId: string): Promise<boolean>;
  setBoardInvitationPreference(
    userId: string,
    accepts: boolean,
  ): Promise<boolean>;
...
export function createAccountService(
  ...
  return {
    ...
    async boardInvitationPreference(userId) {
      const result = await pool.query<{ accepts_board_invitations: boolean }>(
        `SELECT accepts_board_invitations FROM users WHERE id = $1`,
        [userId],
      );
      // An account that has gone missing is treated as accepting nothing
      // rather than as accepting everything.
      return result.rows[0]?.accepts_board_invitations ?? false;
    },

    // Turning offers off refuses new ones only. Boards already joined were
    // consented to separately, and removing them would be a different decision
    // than the one this switch expresses.
    async setBoardInvitationPreference(userId, accepts) {
      const result = await pool.query<{ accepts_board_invitations: boolean }>(
        `UPDATE users SET accepts_board_invitations = $2 WHERE id = $1
         RETURNING accepts_board_invitations`,
        [userId, accepts],
      );
      const row = result.rows[0];
      if (row === undefined) throw new Error('Account not found');
      return row.accepts_board_invitations;
    },
