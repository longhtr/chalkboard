...
export async function runMigrations(
  ...
  return runWithFailurePreservingCleanup(
    ...
    'Database migration operation or advisory-lock cleanup failed',
  );
}
...
async function main(): Promise<void> {
  ...
  await runWithFailurePreservingCleanup(
    ...
    [() => pool.end()],
    'Database migration command or pool cleanup failed',
  );
}
...
if (
  ...
  main().catch((error: unknown) => {
    writeOperationalError('database.migration', error);
    process.exitCode = 1;
  });
}
