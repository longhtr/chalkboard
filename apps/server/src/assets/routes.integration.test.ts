...
import { MAX_ASSET_BYTES } from './validation.js';
...
const maximumPaddingBytes = MAX_ASSET_BYTES - PNG.length;
...
describe.skipIf(connectionString === undefined)('board asset API', () => {
  ...
  it('authorizes, validates, deduplicates, serves, and removes assets', async () => {
    ...
    // Sharing offers access; the offer has to be accepted before either of
    // these accounts is a member and can be judged by the asset routes.
    for (const [email, role, memberCookie] of [
      [requiredTestValue(emails[1], 'editor email'), 'editor', editorCookie],
      [requiredTestValue(emails[2], 'viewer email'), 'viewer', viewerCookie],
    ] as const) {
    ...
    const oversized = await app.inject({
      ...
      url: `/api/boards/${boardId}/assets`,
      headers: { cookie: editorCookie, 'content-type': 'image/png' },
      payload: Buffer.alloc(MAX_ASSET_BYTES + 1),
    });
