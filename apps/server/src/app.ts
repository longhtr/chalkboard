...
import { installEmailFeedbackRoutes } from './api/emailFeedbackRoutes.js';
...
export function buildApp({
  ...
  if (verificationEmailSender === undefined) {
    if (config.nodeEnv === 'production') {
      verificationEmailSender =
        config.verificationEmail === null
          ? createUnavailableVerificationEmailSender()
          : createSesVerificationEmailSender(config.verificationEmail);
    } else {
      const development = createDevelopmentVerificationEmailSender(
        config.publicOrigin ?? 'http://127.0.0.1:5173',
      );
  ...
  if (
    config.verificationEmail !== null &&
    database.emailFeedback !== undefined
  ) {
    ...
    const feedbackRegion = config.verificationEmail.region;
    const feedbackTopicArn = config.verificationEmail.feedbackTopicArn;
    installEmailFeedbackRoutes(app, {
      ...(config.snsConfirmSubscription
        ? {
            confirmSubscription: (envelope) =>
              confirmSnsSubscription(envelope, {
      ...
      verifier: createSnsEnvelopeVerifier({
        region: feedbackRegion,
        topicArn: feedbackTopicArn,
      }),
    });
