...
export class CollaborationRoom {
  ...
  constructor(options: CollaborationRoomOptions) {
    ...
    this.document.on('update', (update: Uint8Array, origin: unknown) => {
      ...
      this.persistenceChain = this.persistenceChain
        ...
        .catch((error: unknown) => {
          const compactionOverload = isErrorInstance(
            error,
            CollaborationCompactionOverloadError,
          );
          if (compactionOverload) {
            this.metrics.recordCollaborationCompactionOverload();
          } else {
            this.metrics.recordStorageFailure();
          }
          this.persistenceFailure = error;
          this.onOperationalError('collaboration.persistence', error);
          for (const socket of this.connections.keys()) {
            socket.close(
              compactionOverload ? 1013 : 1011,
              compactionOverload
  ...
  connect(
    ...
    presenceUserId: string | null = null,
  ): void {
  ...
  receive(socket: WebSocket, message: Uint8Array): void {
    ...
    if (type === COLLABORATION_MESSAGE_SYNC) {
      ...
      if (
        ...
        if (connection === undefined || !connection.admitUpdate()) {
          this.metrics.recordCollaborationPolicyRejection();
          socket.close(1008, 'Collaboration update rate exceeded');
          return;
        }
    ...
    try {
      ...
      if (
        ...
        if (!this.documentAdmission.canCompact(snapshot.byteLength)) {
          if (!this.documentAdmission.canAdmit(0)) {
            // A lowered hard limit leaves legacy over-limit state readable but
            // read-only. Preserve its existing snapshot/tail rather than
            // turning ordinary room retirement into a failed write attempt.
            return;
          }
          this.metrics.recordCollaborationDocumentLimitRejection();
        ...
        try {
          ...
          if (isErrorInstance(error, CollaborationCompactionOverloadError)) {
            this.metrics.recordCollaborationCompactionOverload();
          } else {
            this.metrics.recordStorageFailure();
          }
...
export async function loadCollaborationRoom(
  ...
  try {
    ...
    if (encodedDocumentBytes > documentLimits.loadedBytes) {
      throw new CollaborationDocumentLimitError(
        'Stored Yjs document exceeds the room-load byte limit',
      );
    }
    return new CollaborationRoom({
      ...
      compactionUpdateThreshold,
      document,
      // PostgreSQL charges the current snapshot plus every uncompacted tail
      // update. Use at least that durable byte total for pre-broadcast admission
      // rather than the usually smaller merged Yjs encoding, or the database
      // could reject an update only after peers had already received it.
      documentAdmission: new CollaborationDocumentAdmission(
        Math.max(encodedDocumentBytes, persistedAdmissionBytes),
        documentLimits,
      ),
    ...
    if (isErrorInstance(error, CollaborationDocumentLimitError)) {
