...
export const MAX_NORMAL_COLLABORATION_DOCUMENT_BYTES = 5 * 1_024 * 1_024;

export const DEFAULT_COLLABORATION_DOCUMENT_LIMITS: CollaborationDocumentLimits =
  {
    documentBytes: MAX_NORMAL_COLLABORATION_DOCUMENT_BYTES,
...
export function validatePersistedYjsRoom(
  ...
  if (snapshotBytes > limits.loadedBytes) {
    throw new CollaborationDocumentLimitError(
      'Stored Yjs snapshot exceeds the room-load byte limit',
    );
  }
...
export class CollaborationDocumentAdmission {
  ...
  constructor(
    ...
    if (
      compactedDocumentBytes < 0 ||
      compactedDocumentBytes > limits.loadedBytes
    ) {
      throw new CollaborationDocumentLimitError(
        'Yjs document exceeds the room-load byte limit',
      );
    }
  }
