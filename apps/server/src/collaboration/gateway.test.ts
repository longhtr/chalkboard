...
import {
  ...
  MAX_DEMO_COLLABORATION_CONNECTIONS_PER_USER,
} from './gateway.js';
import {
...
describe('collaboration gateway', () => {
  ...
  it('bounds aggregate connections for one shared demo identity', async () => {
    const app = buildApp({
      config: loadConfig({ DATABASE_URL: 'postgresql://unused/test' }),
      database: {
        close: async () => undefined,
    ...
    try {
      for (
        let index = 0;
        index < MAX_DEMO_COLLABORATION_CONNECTIONS_PER_USER;
        index += 1
      ) {
        ...
        const socket = new WebSocket(url, {
          headers: {
            Cookie: `chalkboard_session=demo-session-${sessionNumber}`,
          },
        });
        sockets.push(socket);
      ...
      const rejected = new WebSocket(url, {
        headers: { Cookie: 'chalkboard_session=demo-session-overflow' },
      });
      await expect(
        new Promise<void>((resolve, reject) => {
