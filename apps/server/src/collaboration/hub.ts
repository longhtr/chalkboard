...
export class CollaborationHub {
  ...
  private createRoom(boardId: string): CollaborationRoomSlot {
    const slot: CollaborationRoomSlot = {
      ...
      room: loadCollaborationRoom({
        ...
        documentLimits: this.documentLimits,
  ...
  private retireIfEmpty(
    ...
    const retirement = room.destroy().catch(() => {
      // Room destruction records persistence failure before rejecting; retire it.
    });
  ...
  async connect(
    ...
    try {
      while (!closed && !this.destroying) {
        const slot = this.rooms.get(boardId) ?? this.createRoom(boardId);
        ...
        socket.off('message', queueMessage);
        try {
          room.connect(socket, queuedMessages, canEdit);
          queuedMessages.length = 0;
          queuedMessageBytes = 0;
        } catch {
          this.metrics.recordCollaborationPolicyRejection();
    ...
    await Promise.all(
      slots.map(async (slot) => {
        ...
        await room?.destroy().catch(() => {
          // Continue draining independent rooms after a recorded room failure.
        });
      }),
    );
    this.rooms.clear();
