...
/**
 * Presence identity is only worth trusting if the server binds it to the
 * authenticated connection. A browser hides presence belonging to its own
 * account, so a peer free to publish somebody else's identifier would erase its
 * own cursor, selection, and avatar from that person's view alone. Presence
 * that claims no account stays acceptable; only a mismatched claim is refused.
 */
function ownsPresenceIdentity(state: unknown, userId: string): boolean {
  ...
  const claimed = (user as { id?: unknown }).id;
  return claimed === undefined || claimed === userId;
}
...
export class CollaborationAwarenessAdmission<TOwner extends object> {
  ...
  inspect(
    ...
    /** Account behind this connection, or null when none was authenticated. */
    ...
    for (const entry of entries) {
      ...
      if (
        presenceUserId !== null &&
        !ownsPresenceIdentity(entry.state, presenceUserId)
      ) {
        throw new CollaborationAwarenessAdmissionError(
          'policy',
          'Awareness presence claims another account',
        );
      }
