  ...
  return {
    ...
    async appendUpdate(boardId, update) {
      try {
        ...
        if (sequence === undefined)
          throw new Error('Yjs update was not stored');
    ...
    async compact(boardId, snapshot, throughSequence) {
      ...
      try {
        ...
        // Remove only the covered tail before replacing the snapshot. The
        // transaction is atomic, and this order lets quota ledgers replace
        // bytes instead of temporarily double-counting compacted state.
        await client.query(
