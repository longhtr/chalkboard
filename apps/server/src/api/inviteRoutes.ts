...
export function installBoardInviteRoutes(
  ...
  app.post('/api/board-invites/redeem', async (request, reply) => {
    ...
    const redemption = await boards.redeemInviteLink(
      user.id,
      tokenHash(parsed.data.token),
    );
    if (redemption.outcome === 'partition-mismatch') {
      writeSecurityAuditEvent(request, {
      ...
      // 403 separates "you may not" from 404's "no such link"; the client words
      // the refusal from its own session.
      return reply.code(403).send({
        error: 'This invitation cannot be opened by this account.',
      });
    }
    if (redemption.outcome === 'not-found') {
      writeSecurityAuditEvent(request, {
        action: 'invite-link.redeem',
        actorUserId: user.id,
        outcome: 'rejected',
