...
/**
 * Phrases a wait the reader can act on. Rounded up and floored at one minute so
 * a countdown never reads "0 minutes" and never invites an immediate retry that
 * the same limit would refuse again.
 */
function inMinutes(milliseconds: number): string {
...
const boardInvitationPreferenceSchema = z.object({
  acceptsBoardInvitations: z.boolean(),
});
...
export function installAccountRoutes(
  ...
  app.get('/api/account/board-invitations', async (request, reply) => {
    const user = await authenticatedUser(request, reply, accounts, metrics);
    if (user === null) return;
    return {
      acceptsBoardInvitations: await accounts.boardInvitationPreference(
        user.id,
      ),
    };
  });

  app.patch('/api/account/board-invitations', async (request, reply) => {
    const user = await authenticatedUser(request, reply, accounts, metrics);
    if (user === null) return;
    const parsed = boardInvitationPreferenceSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.code(400).send({ error: 'Invalid preference' });
    }
    return {
      acceptsBoardInvitations: await accounts.setBoardInvitationPreference(
        user.id,
        parsed.data.acceptsBoardInvitations,
      ),
    };
  });
    ...
    try {
      ...
      if (result.outcome !== 'accepted') {
          ...
          reason:
            result.outcome === 'invalid-password'
              ...
              : result.outcome === 'conflict' ||
                  result.outcome === 'pending-other-destination'
                ? 'conflict'
                : result.outcome === 'limited'
                  ? 'rate-limited'
                  : result.outcome === 'suppressed-destination'
        ...
        if (result.outcome === 'conflict') {
          // A free address returns 202 and a taken one 409, so the status
          // already distinguishes the two. Vague wording bought no privacy
          // here and only left the caller without a next step.
          return reply.code(409).send({
            error: 'That address is already in use.',
          });
        }
        if (result.outcome === 'pending-other-destination') {
          // Naming the held address is safe: the caller is authenticated and
          // has just re-entered the current password. The wait is quoted in
          // minutes because "wait until it expires" leaves the reader with no
          // idea whether that means seconds or an hour.
          return reply.code(409).send({
            error: `You already have a pending email change to ${result.destination}. Enter the code sent there, or try again in ${inMinutes(Date.parse(result.expiresAt) - Date.now())} once it expires.`,
        ...
        if (result.outcome === 'limited') {
          ...
          return reply
            ...
            .code(429)
            .send({
              error: `Too many email changes. Try again in ${inMinutes(retryAfterSeconds * 1_000)}.`,
            });
        }
        ...
        if (result.outcome === 'suppressed-destination') {
          // Permanent: retrying can never succeed, so this must not arrive as
          // a 503 that invites the caller to reconnect and try again.
          return reply.code(422).send({
            error:
              'This address previously bounced or reported our mail as spam, so we cannot send to it. Use a different address.',
          });
        }
