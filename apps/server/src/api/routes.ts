...
export function installApiRoutes(
  ...
  app.addHook('onRequest', async (request, reply) => {
    // The email provider is not a browser and sends no Origin header. This
    // exact endpoint authenticates every request with its own signature before
    // any event is parsed, so an origin check would only reject valid webhooks.
    // Compared against the path alone: `request.url` carries any query string,
    // and an exact match on the whole value would send a webhook that arrived
    // with one into the origin check, which refuses every request without an
    // Origin header.
    if (request.url.split('?')[0] === '/api/email-feedback/resend') return;
    if (
  ...
  app.addHook('preHandler', async (request, reply) => {
    ...
    if (
      ...
      !(
        ...
        path.startsWith('/api/board-invites/') ||
        path.startsWith('/api/board-invitations/') ||
