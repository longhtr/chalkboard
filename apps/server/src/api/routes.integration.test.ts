...
import { MAX_DEMO_API_MUTATIONS_PER_MINUTE } from './routes.js';
...
describe.skipIf(connectionString === undefined)('account and board API', () => {
  ...
  const demoRateEmail = `demo-rate-${crypto.randomUUID()}@chalkboard.test`;
  ...
  it('registers, restores a session, and manages board metadata', async () => {
    ...
    const malformedInvitation = await app.inject({
      method: 'POST',
      url: '/api/board-invitations/not-a-uuid/accept',
      headers: { cookie },
    });
    expect(malformedInvitation.statusCode).toBe(400);
    expect(malformedInvitation.json()).toEqual({
      error: 'Invalid resource identifier',
    });
    ...
    // This collaborator redeemed an invite link above, so they have already
    // consented to the board. Sharing by email re-roles them outright rather
    // than offering something they cannot refuse without leaving.
    ...
    expect(added.json()).toMatchObject({ member: { role: 'viewer' } });
    ...
    // Somebody who has never joined is offered the board instead. Until they
    // answer, the board is theirs to decide about and nothing more.
    const inviteeRegistration = await app.inject({
      method: 'POST',
      url: '/api/auth/register',
      payload: {
        displayName: 'Ada',
        email: inviteeEmail,
        password: 'a third correct password',
      },
    });
    expect(inviteeRegistration.statusCode).toBe(202);
    const inviteeCookie = responseCookie(
      (
        await app.inject({
          method: 'POST',
          url: '/api/auth/verify-email',
          payload: { code: '1234-5678', email: inviteeEmail },
        })
      ).headers['set-cookie'],
    );
    ...
    expect(offered.json()).toMatchObject({
      invitation: { boardId, role: 'editor', title: 'Analysis notes' },
    });

    const beforeAnswer = await app.inject({
      method: 'GET',
      url: '/api/boards',
      headers: { cookie: inviteeCookie },
    });
    expect(beforeAnswer.json()).toMatchObject({ boards: [] });
    ...
    expect(pending.json()).toMatchObject({
      invitations: [{ boardId, role: 'editor' }],
    });

    const declined = await app.inject({
      method: 'POST',
      url: `/api/board-invitations/${boardId}/reject`,
      headers: { cookie: inviteeCookie },
    });
    ...
    ).toMatchObject({ invitations: [] });
    // Declining leaves no membership behind, so a second answer has nothing
    // to consume.
    expect(
      (
        await app.inject({
          method: 'POST',
          url: `/api/board-invitations/${boardId}/accept`,
          headers: { cookie: inviteeCookie },
        })
      ).statusCode,
    ).toBe(404);
    ...
    // The owner can see the offer while it waits, and take it back.
    await app.inject({
      method: 'POST',
      url: `/api/boards/${boardId}/members`,
      headers: { cookie },
    ...
    ).toMatchObject({ invitations: [{ email: inviteeEmail, role: 'viewer' }] });
    // Somebody who is not the owner has no business reading who was offered
    // the board, so the list is refused rather than returned empty.
    expect(
      (
        await app.inject({
          method: 'GET',
    ...
    const inviteeId = requiredTestString(
      (
        await app.inject({
          method: 'GET',
          url: `/api/boards/${boardId}/invitations`,
      ...
      ).json().invitations?.[0]?.userId,
      'pending invitee identifier',
    );
    expect(
      (
        await app.inject({
          method: 'DELETE',
          url: `/api/boards/${boardId}/invitations/${inviteeId}`,
          headers: { cookie },
        })
      ).statusCode,
    ).toBe(204);
    ...
    // Switching offers off refuses them at the moment they are attempted, so
    // the owner is told rather than left waiting on an answer.
    expect(
      (
        await app.inject({
          method: 'PATCH',
          url: '/api/account/board-invitations',
          headers: { cookie: inviteeCookie },
          payload: { acceptsBoardInvitations: false },
        })
      ).json(),
    ).toMatchObject({ acceptsBoardInvitations: false });
    ...
    await app.inject({
      ...
      url: '/api/account/board-invitations',
      headers: { cookie: inviteeCookie },
      payload: { acceptsBoardInvitations: true },
    });
    await app.inject({
      method: 'POST',
      url: `/api/boards/${boardId}/members`,
    ...
    ).toMatchObject({ boards: [{ id: boardId, role: 'editor' }] });

    // An owner's decision about a role is the owner's to reverse. Redeeming a
    // read-and-edit link must not hand a demoted member a way to put the role
    // back themselves.
    expect(
      (
        await app.inject({
          method: 'PATCH',
          url: `/api/boards/${boardId}/members/${inviteeId}`,
          headers: { cookie },
          payload: { role: 'viewer' },
        })
      ).statusCode,
    ).toBe(200);
    const pinnedLink = await app.inject({
      method: 'POST',
      url: `/api/boards/${boardId}/invite-links`,
      headers: { cookie },
      payload: { role: 'editor' },
    });
    const pinnedToken = requiredTestString(
      pinnedLink.json().token,
      'editor invitation token',
    );
    // Redemption still succeeds and still opens the board; it simply leaves
    // the owner-assigned role alone.
    expect(
      (
        await app.inject({
          method: 'POST',
          url: '/api/board-invites/redeem',
          headers: { cookie: inviteeCookie },
          payload: { token: pinnedToken },
        })
      ).json(),
    ).toMatchObject({ board: { id: boardId, role: 'viewer' } });
    expect(
      (
        await app.inject({
          method: 'GET',
    ...
    ).toMatchObject({ boards: [{ id: boardId, role: 'viewer' }] });
    // The owner can still move them, because the decision was always theirs.
    await app.inject({
      method: 'PATCH',
      url: `/api/boards/${boardId}/members/${inviteeId}`,
      headers: { cookie },
    ...
    // Having joined, the way back out is the member's own to take.
    const left = await app.inject({
      method: 'DELETE',
      url: `/api/boards/${boardId}/membership`,
      headers: { cookie: inviteeCookie },
    });
    ...
    // A board with no owner would have nobody who could delete it or say who
    // else may see it, so the owner is refused.
    expect(
      (
        await app.inject({
          method: 'DELETE',
    ...
    // Trashing a board invalidates acceptance but must not consume the offer:
    // restoring the board restores the invitee's ability to decide.
    const restorableOfferBoard = await app.inject({
      method: 'POST',
      url: '/api/boards',
      headers: { cookie },
      payload: { title: 'Restorable offer' },
    });
    const restorableOfferBoardId = requiredTestString(
      restorableOfferBoard.json().board?.id,
      'restorable offer board identifier',
    );
    expect(
      (
        await app.inject({
          method: 'POST',
          url: `/api/boards/${restorableOfferBoardId}/members`,
          headers: { cookie },
          payload: { email: inviteeEmail, role: 'viewer' },
        })
      ).statusCode,
    ...
    expect(
      (
        await app.inject({
          method: 'DELETE',
          url: `/api/boards/${restorableOfferBoardId}`,
          headers: { cookie },
        })
      ).statusCode,
    ).toBe(204);
    expect(
      (
        await app.inject({
          method: 'POST',
          url: `/api/board-invitations/${restorableOfferBoardId}/accept`,
          headers: { cookie: inviteeCookie },
        })
      ).statusCode,
    ).toBe(404);
    expect(
      (
        await pool.query(
          ...
          [restorableOfferBoardId, inviteeId],
        )
      ).rowCount,
    ).toBe(1);
    await app.inject({
      method: 'POST',
      url: `/api/boards/${restorableOfferBoardId}/restore`,
      headers: { cookie },
    });
    expect(
      (
    ...
    await app.inject({
      method: 'DELETE',
      url: `/api/boards/${restorableOfferBoardId}/permanent`,
      headers: { cookie },
    });
  ...
  it('verifies registration, email changes, password changes, and password recovery', async () => {
    ...
    const otherDestination = await app.inject({
      method: 'PATCH',
      url: '/api/account/email',
      headers: { cookie: settingsCookie },
      payload: {
    ...
    expect(otherDestination.statusCode).toBe(409);
    expect(otherDestination.json()).toMatchObject({
      pendingDestination: changedSettingsEmail,
    });
    const pendingAfterOtherDestination = await pool.query<{
      code_hash: string;
      generation_id: string;
      send_intent_id: string;
    }>(
    ...
    expect(pendingAfterOtherDestination.rows).toEqual(pendingEmailChange.rows);
  ...
  it('applies demo mutation limits to invitation answers', async () => {
    const salt = Buffer.from('demo-rate-test-salt');
    const derived = scryptSync('demo rate password', salt, 64);
    ...
    await pool.query(
      `INSERT INTO users
       ...
       VALUES ($1, $1, 'Demo Rate User', $2, TRUE)`,
      [demoRateEmail, passwordHash],
    );
    const login = await app.inject({
      method: 'POST',
      url: '/api/auth/login',
      payload: { email: demoRateEmail, password: 'demo rate password' },
    });
    const demoCookie = responseCookie(login.headers['set-cookie']);
    const missingBoardId = crypto.randomUUID();
    for (
      let attempt = 0;
      attempt < MAX_DEMO_API_MUTATIONS_PER_MINUTE;
      attempt += 1
    ) {
      expect(
        (
          await app.inject({
            method: 'POST',
            url: `/api/board-invitations/${missingBoardId}/reject`,
