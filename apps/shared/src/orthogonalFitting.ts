...
/**
 * Coordinates that a run shares are written by the same arithmetic on both
 * vertices, so exact equality normally holds. This tolerance only absorbs drift
 * accumulated by repeated affine edits; it stays far below one device pixel at
 * any usable zoom, so a genuinely diagonal run is never mistaken for an
 * axis-aligned one.
 */
const ORTHOGONAL_RUN_EPSILON = 1e-6;

/**
 * Moves one vertex of an axis-aligned polyline and pulls its neighbours so every
 * run stays horizontal or vertical.
 *
 * A corner joins one horizontal and one vertical run. Moving it would break both
 * unless each neighbour follows the coordinate its run shares with the corner:
 * the neighbour across a horizontal run follows `y`, the neighbour across a
 * vertical run follows `x`. The other coordinate is absorbed by that run's own
 * length.
 *
 * Vertex count is deliberately preserved even when a drag collapses a run to
 * zero length, because handle indexes must stay stable for the whole pointer
 * lifetime. A run that is already neither horizontal nor vertical is left alone
 * rather than snapped, so damaged geometry is never silently rewritten.
 */
export function moveOrthogonalVertex(
  ...
  const moved = vertices.map((vertex) => ({ ...vertex }));
  ...
  for (const neighbourIndex of [index - 1, index + 1]) {
    const neighbour = moved[neighbourIndex];
    if (neighbour === undefined) continue;
    // The run's original orientation decides what follows, so both tests read
    // the pre-move corner rather than the already-updated one.
    if (Math.abs(neighbour.y - origin.y) <= ORTHOGONAL_RUN_EPSILON) {
      neighbour.y = point.y;
    }
    if (Math.abs(neighbour.x - origin.x) <= ORTHOGONAL_RUN_EPSILON) {
      neighbour.x = point.x;
    }
  }
  return moved;
