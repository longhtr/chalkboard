...
describe('shape fill geometry', () => {
  ...
  it('encloses a rectangle and an ellipse for filling', () => {
    ...
    const halfEllipse = shapeFillPolygon('ellipse', bounds, {
      ellipseEndAngle: 180,
      ellipseStartAngle: 0,
    });
    expect(halfEllipse.length).toBeGreaterThan(8);
    expect(halfEllipse[0]).toEqual({ x: 90, y: 40 });
    expect(halfEllipse.at(-1)?.x).toBeCloseTo(10);
    expect(halfEllipse.at(-1)?.y).toBeCloseTo(40);

    const extendedHalf = shapeFillPolygon('ellipse', bounds, {
      ellipseEndAngle: 180,
      ellipseStartAngle: 0,
      strokeWidth: 4,
    });
    expect(extendedHalf.at(-2)).toEqual({ x: 10, y: 38 });
    expect(extendedHalf.at(-1)).toEqual({ x: 90, y: 38 });
  });
});
