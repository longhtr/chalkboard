...
describe('bounds', () => {
  ...
  it('grows a turned element box to cover what it now covers', () => {
    // 100 x 50 at (10, 20), so its middle is (60, 45). Stood on end it is 50
    // wide and 100 tall around that same middle.
    expect(rotatedElementBounds({ ...rectangle, rotation: 90 })).toEqual({
      x: 35,
      y: -5.000000000000002,
      width: 50,
      height: 100,
    });
    // An upright element is handed back untouched, so nothing pays for trig it
    // does not need.
    expect(rotatedElementBounds(rectangle)).toEqual(elementBounds(rectangle));
  });

  it('encloses a whole selection by what each piece covers turned', () => {
    const turned = { ...rectangle, id: 'turned', rotation: 90 };
    const far = { ...rectangle, id: 'far', x: 200, y: 200 };
    expect(rotatedSelectionBounds([])).toBe(null);
    const bounds = rotatedSelectionBounds([turned, far]);
    expect(bounds?.x).toBeCloseTo(35, 6);
    expect(bounds?.y).toBeCloseTo(-5, 6);
    expect(bounds?.width).toBeCloseTo(265, 6);
    expect(bounds?.height).toBeCloseTo(255, 6);
  });
...
describe('element operations', () => {
  ...
  it('bounds and hit-tests straight and editable Bézier paths', () => {
    ...
    const connectedStraight = {
      ...line,
      height: 70,
      pathKind: 'straight' as const,
      segments: polylineCubicSegments(vertices, vertices[0] ?? { x: 0, y: 0 }),
      straightSegmented: true as const,
    };
    expect(linePathPoints(connectedStraight)).toEqual(vertices);
    expect(elementBounds(connectedStraight)).toEqual({
      height: 70,
      width: 100,
      x: 10,
      y: 20,
    });
    expect(hitTestElement(connectedStraight, { x: 110, y: 60 }, 1)).toBe(true);

    const { straightSegmented: legacyMarker, ...legacyStraight } =
      connectedStraight;
    void legacyMarker;
    expect(linePathPoints(legacyStraight)).toEqual([
      { x: 10, y: 20 },
      { x: 110, y: 90 },
    ]);
  });
  ...
  it('hit-tests ellipse, triangle, and diamond geometry', () => {
    ...
    expect(hitTestElement(lowerHalf, { x: 60, y: 70 }, 0.1)).toBe(true);
    expect(hitTestElement(lowerHalf, { x: 60, y: 60 }, 0.1)).toBe(false);
    expect(hitTestElement(lowerHalf, { x: 60, y: 30 }, 0.1)).toBe(false);
    const wrappedArc = {
      ...shape,
      ellipseEndAngle: 60,
      ellipseStartAngle: 300,
    };
    expect(hitTestElement(wrappedArc, { x: 110, y: 45 }, 0.1)).toBe(true);
    expect(hitTestElement(wrappedArc, { x: 100, y: 45 }, 0.1)).toBe(false);
    expect(hitTestElement(wrappedArc, { x: 10, y: 45 }, 0.1)).toBe(false);
    const filledWrappedArc = {
      ...wrappedArc,
      backgroundColor: '#a5d8ff',
    };
    expect(hitTestElement(filledWrappedArc, { x: 100, y: 45 }, 0.1)).toBe(true);
    expect(hitTestElement(filledWrappedArc, { x: 84.5, y: 45 }, 0.1)).toBe(
      true,
    );
    expect(hitTestElement(filledWrappedArc, { x: 60, y: 45 }, 0.1)).toBe(false);
    expect(
