...
test('updates account identity and recovers a forgotten password', async ({
  ...
  const emailVerification = page.getByRole('dialog', {
    name: 'Verify new email',
  });
  await expect(emailVerification).toBeVisible();
  await expect(
    emailVerification.getByText(/expires in 15 minutes/u),
  ).toBeVisible();
  await emailVerification.getByLabel('Verification code').fill('1234-5678');
  await emailVerification.getByRole('button', { name: 'Verify email' }).click();
  await expect(emailVerification).toBeHidden();
  ...
  await expect(page.getByText(/expires in 15 minutes/u)).toBeVisible();
...
test('completes the visible account lifecycle and restores the session', async ({
  ...
  const registration = page.getByRole('dialog', { name: 'Create account' });
  const displayName = registration.getByLabel('Display name', { exact: true });
  const registrationEmail = registration.getByLabel('Email', { exact: true });
  await displayName.fill('Ada Browser');
  await registrationEmail.fill(email);
  await expect(registrationEmail).toHaveValue(email);
  await expect(displayName).toHaveValue('Ada Browser');
  await registration
    .getByLabel('Password', { exact: true })
    .fill('correct horse');
  await registration.getByLabel('Confirm password').fill('correct horse');
  await registration.getByLabel('I am not a robot').check();
  ...
  await expect(page).toHaveURL(/\/boards\/[^/]+$/u);
  await expect(page).not.toHaveURL(cloudBoardUrl);
  await expect(
    library.getByRole('button', {
      name: 'Open cloud board Untitled board',
    }),
  ).toBeVisible();
  ...
  ).toHaveText('Restored all cloud boards.');
  ...
  await expect(
    library.locator('.local-board-library__announcement'),
  ).toHaveText('Emptied cloud trash.');
  await expect(library.getByRole('list', { name: 'Cloud trash' })).toHaveCount(
    0,
  );
