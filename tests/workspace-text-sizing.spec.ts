...
test('accepts text size 200 and line spacing 5 as the upper limits', async ({
  page,
}) => {
  await page.goto('/');
  await workspace.selectMixedTextTool(page);
  ...
  await expect(textSizeSlider).toHaveAttribute('max', '200');
  await expect(lineSpacingSlider).toHaveAttribute('max', '5');
  await textSizeInput.fill('200');
  await lineSpacingInput.fill('5');
  await expect(textSizeSlider).toHaveValue('200');
  await expect(lineSpacingSlider).toHaveValue('5');
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        ) as { fontSize?: number; lineSpacing?: number }[];
        return elements[0];
      }),
    )
    .toMatchObject({ fontSize: 200, lineSpacing: 5 });
});
...
test('groups a font-size slider drag into one object undo step', async ({
  page,
}) => {
