...
const MAXIMUM_CLOUD_IMAGE = padPngToBytes(CLOUD_IMAGE, MAX_ASSET_BYTES);
...
test('copies a local board and its image to a durable cloud destination', async ({
  ...
  const boardId = requiredString(
    destinationUrl.split('/').at(-1),
    'copied cloud board identifier',
  );
  const addedObserver = await context.request.post(
    `/api/boards/${boardId}/members`,
    { data: { email: observerEmail, role: 'viewer' } },
  );
  expect(addedObserver.status()).toBe(201);
  await acceptBoardInvitation(observerContext.request, boardId);
...
test('synchronizes a shared cloud board between authenticated accounts', async ({
  ...
  await ownerPage.getByLabel('Choose image or SVG file').setInputFiles({
    buffer: Buffer.alloc(MAX_ASSET_BYTES + 1),
    mimeType: 'image/png',
    name: 'too-large.png',
  });
  await expect(ownerPage.getByRole('alert')).toHaveText(
    new RegExp(
      `Images must be smaller than ${MAX_ASSET_BYTES / 1_000_000} MB`,
      'u',
    ),
  );
  await expect(ownerPage.getByText('Canvas contains 6 objects')).toBeVisible();
  ...
  const revokedBoardNotice = editorPage.getByRole('alert');
  // Invalidation can proactively discover the revoked access, or a reload can
  // be the first request that discovers it. Do not reload after the proactive
  // path: that would discard the warning before asserting the user saw it.
  await Promise.race([
    revokedBoardNotice.waitFor({ state: 'visible', timeout: 1_000 }),
    editorPage.waitForURL(
      (url) => !url.pathname.endsWith(`/boards/${boardId}`),
      {
        timeout: 1_000,
      },
    ),
  ...
  if (
    !(await revokedBoardNotice.isVisible()) &&
    editorPage.url().endsWith(`/boards/${boardId}`)
  ) {
    await editorPage.reload();
  }
  // Contains rather than equals: the alert carries its own Dismiss control.
  await expect(revokedBoardNotice).toContainText(
