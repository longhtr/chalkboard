...
test('copies a local board and its image to a durable cloud destination', async ({
  ...
  await expect(boardLibrary.getByRole('status')).toHaveText(
    'Copied Local cloud source to local boards.',
  );
  ...
  await page
    ...
    .getByRole('button', { name: 'Open Local cloud source' })
    .last()
    .click();
  await expect(
    page.locator('[data-mixed-text-id="local-cloud-equation"]'),
...
test('synchronizes a shared cloud board between authenticated accounts', async ({
  ...
  if (ownerCanvasBounds !== null) {
    ...
    await viewerPage.getByRole('button', { name: 'Board objects' }).click();
  ...
  if (editorCanvasBounds !== null) {
    ...
    await editorPage.getByRole('button', { name: 'Board objects' }).click();
    await editorPage
      .getByRole('button', { name: /Freehand stroke, object 1/ })
      .click();
    await expect(
  ...
  await ownerPage.getByRole('button', { name: 'Board objects' }).click();
  ...
  await expect(
    editorPage.getByText('This cloud board is unavailable'),
  ).toBeVisible();
  await expect(editorPage).toHaveURL(`/boards/${boardId}`);
  // Revoked access falls back to this browser's own local board rather than
  // retaining the cloud board, so local work survives and the removed
  // collaborator keeps nothing of the board they lost. `App.test.tsx` fixes the
  // same contract for a direct route that was never authorized.
  await expect(
    editorPage.getByRole('textbox', { name: 'Board title' }),
