...
test('offers the monitored Contact address from the board menu', async ({
  page,
}) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Open board menu' }).click();
  ...
  await expect(contact).toHaveAttribute(
    'href',
    'mailto:support@chalkboard.space',
  );
  await expect(contact).toHaveCSS('text-decoration-line', 'none');
});
...
test('opens a rendered MathLive and LaTeX cheatsheet from the board menu', async ({
  ...
  // The examples go through the same decoration an inactive block does, so the
  // layout corrections have something to match. Without it the reference drew
  // a command one way and the board drew it another, which is worse than no
  // example at all. Eleven of the seventeen accents shown here are corrected;
  // the six that are not carry no hook by design.
  await expect(
    dialog.locator('.latex-cheatsheet-entry__preview [data-excalifont-accent]'),
  ).toHaveCount(11);
  await expect(
    dialog.locator(
      '.latex-cheatsheet-entry__preview [data-excalifont-op="integral"]',
    ),
  ).toHaveCount(4);
  await expect(
...
test('remembers Board objects visibility across tool changes', async ({
  page,
}) => {
...
test('keeps board, export, and keyboard dialog text readable', async ({
  ...
  await expect(
    dialog.getByRole('heading', { name: 'Touch gestures' }),
  ).toBeVisible();
  const sectionHeadings = await dialog
    .locator('.shortcut-section h3')
    .allTextContents();
  expect(sectionHeadings.indexOf('Touch gestures')).toBe(
    sectionHeadings.indexOf('Board objects') + 1,
  );
  await expect(
    dialog.getByRole('heading', { name: 'Mixed text' }),
  ).toBeVisible();
  await expect(
    dialog.getByRole('heading', { name: 'Windows & dialogs' }),
  ).toBeVisible();
  await expect(
    dialog.getByText(/With empty Selection.*latest creation or deletion/u),
  ).toBeVisible();
  await expect(dialog.getByText(/disabled in Drag canvas/u)).toBeVisible();
  for (const binding of [
    'Undo current history target',
    'Redo current history target',
    'Redo current history target (alternative)',
    'Copy selected objects',
    'Paste copied objects',
    'Delete selection',
    'Move selected objects (1 px; 10 px with Shift)',
    'Start the next connected Straight line',
    'Finish path handle editing',
    'Add a connected Straight segment',
    'Use Shift while drawing, resizing, or rotating',
    'Switch input mode',
    'Switch between rendered and source editing',
    'Previous / next typing color',
    'Decrease / increase text size',
    'Decrease / increase line spacing',
    'Navigate between blocks',
    'Select all writing',
    'Copy / cut / paste writing',
    'Complete a LaTeX command',
    'Accept a command / start a row',
    'Move to the next placeholder',
    'Cancel an unfinished command',
    'Close the active dialog or panel',
    'Apply / dismiss a custom color',
    'Commit / revert a numeric field',
    'Remove a focused custom color',
  ]) {
    await expect(dialog.getByText(binding, { exact: true })).toBeVisible();
  }
  // Two rows people otherwise have to discover by accident, each checked in the
  // section it belongs to: switching editing views only applies inside a mixed
  // text block, so listing it under general editing would be a small lie.
  await expect(
    dialog
      .locator('.shortcut-section', { hasText: 'Mixed text' })
      .getByText('Switch between rendered and source editing'),
  ).toBeVisible();
  await expect(
    dialog
      .locator('.shortcut-section', { hasText: 'Editing' })
      .getByText('Snap rotation to 15°'),
  ).toBeVisible();
  // Shift means three different things depending on what is being dragged, so
  // each row says which, rather than listing one chord twice with no way to
  // tell the entries apart.
  const editing = dialog.locator('.shortcut-section', { hasText: 'Editing' });
  await expect(
    editing.getByText('Constrain shape while drawing'),
  ).toBeVisible();
  await expect(
    editing.getByText('Keep proportions while resizing'),
  ).toBeVisible();
  await expect(
    editing.getByText('Move selected objects (1 px; 10 px with Shift)'),
  ).toBeVisible();
  // Shift-click reads under both headings on purpose: it selects on the canvas
  // and in the objects list, and each heading says which is meant, so neither
  // row needs the qualifying words the three Shift-drag rows share a section
  // with and therefore do need.
  await expect(editing.getByText('Add or remove from selection')).toBeVisible();
  const boardObjects = dialog.locator('.shortcut-section', {
    hasText: 'Board objects',
  });
  await expect(
    boardObjects.getByText('Add or remove from selection'),
  ).toBeVisible();
  await expect(
    boardObjects.getByText('Move selected objects', { exact: false }),
  ).toHaveCount(0);
  // Dragging an entry moves the whole selection through the element order,
  // which is the draw order, so it is named the way the panel's own buttons
  // name it rather than as a vaguer reorder.
  await expect(boardObjects.getByText('Change layer order')).toBeVisible();
