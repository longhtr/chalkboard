/** Local board create, open, rename, duplicate, import, trash, restore, permanent delete, and tab workflows. */
...
test('keeps the board window open when the open board is trashed', async ({
  ...
  await expect(
    currentEntry.getByRole('button', { name: 'Open Open board' }),
  ).toBeVisible();
...
test('clears the bottom-right storage warning after persistence recovers', async ({
  ...
  await expect(warning).toHaveCSS('position', 'fixed');
  expect(
...
test('opens a keyboard shortcut reference from the board menu', async ({
  page,
}) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Open board menu' }).click();
  ...
  await expect(dialog.getByText('Switch input mode')).toBeVisible();
  await expect(
