...
test('publishes substantive privacy, terms, acceptable-use, and contact routes', async ({
  page,
}) => {
  for (const [path, heading] of [
    ...
    await expect(
      page.locator('.public-information__article p').first(),
    ).toHaveCSS('font-size', '16px');
    await expect(
      page.locator('.public-information__article h2').first(),
