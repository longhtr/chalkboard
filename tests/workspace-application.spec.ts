...
test('migrates legacy singleton cache state to one generated board ID', async ({
  ...
  await expect(
    page.getByRole('button', { name: 'Open Legacy board' }),
  ).toHaveCount(1);
});
...
test('lists the pristine default local board before the first edit', async ({
  ...
  await expect(
    library.getByRole('button', { name: 'Open Untitled board' }),
  ).toBeVisible();
...
test('creates and reopens multiple local boards without an account', async ({
  ...
  await expect(
    library.getByRole('button', { name: 'Open First board' }),
  ).toBeVisible();
  await expect(
    library.getByRole('button', { name: 'Open Second board' }),
  ).toBeVisible();

  await library.getByRole('button', { name: 'Open First board' }).click();
  ...
  await expect(
    secondPage.getByRole('button', { name: 'Open Renamed first board' }),
  ).toBeVisible();
...
test('moves local boards through durable trash, restores all, and empties trash', async ({
  ...
  const recoverEntry = library
    .getByRole('button', { name: 'Open Recover me' })
    .locator('..');
  await recoverEntry.getByRole('button', { name: 'Trash' }).click();
  await expect(library.getByRole('status')).toHaveText(
    'Moved Recover me to trash.',
  );
  await expect(
    library.getByRole('button', { name: 'Open Recover me' }),
  ).toHaveCount(0);
  ...
  await expect(library.getByRole('status')).toHaveText('Emptied trash.');
  await expect(
    library.getByRole('button', { name: 'Open Recover me' }),
  ).toHaveCount(0);
  await expect(library.getByRole('list', { name: 'Device trash' })).toHaveCount(
...
test('changes and persists grid spacing and dot size', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Open board menu' }).click();
  ...
  await expect(page.getByText('37px', { exact: true })).toBeVisible();
  await expect
    .poll(() =>
      page.evaluate(() => localStorage.getItem('chalkboard:grid-spacing')),
    )
    .toBe('37');
  await dotSize.fill('2.25');
  await expect(page.getByText('2.25px', { exact: true })).toBeVisible();
  await expect
    .poll(() =>
      page.evaluate(() => localStorage.getItem('chalkboard:grid-dot-size')),
    )
    .toBe('2.25');
  await expect(
  ...
  await expect(dotSize).toHaveCount(0);
