...
test('renders mixed text and edits only the clicked delimited math region', async ({
  ...
  await expect
    .poll(async () => {
      finalActiveMath = await activeMathLatex(mathField, 1);
      return finalActiveMath;
    })
    .toContain('+1');
  ...
  await expect
    .poll(async () => {
      updatedSource = await page
        ...
        .getAttribute('aria-label');
      if (updatedSource === null) return null;
      return [...updatedSource.matchAll(/\${1,2}([\s\S]*?)\${1,2}/gu)][1]?.[1];
    })
    .toBe(finalActiveMath);
...
test('double-clicking an active equation selects the whole line', async ({
  page,
}) => {
  await page.goto('/');
  const { bounds } = await canvasBounds(page);
  ...
  // A double press takes the line it lands on, which for an equation on one
  // line is the whole of it - never MathLive's own idea of the symbol under
  // the pointer. Repeated because pressing again on an already selected line
  // used to be where the field lost its caret entirely.
  for (let index = 0; index < 10; index += 1) {
    ...
    await expect
      .poll(() =>
        mathField.evaluate((field) => {
          ...
          return range === undefined
            ? null
            : {
                coversLine: range[0] === 0 && range[1] === field.lastOffset,
                marked:
      ...
      .toEqual({ coversLine: true, marked: true });
  }

  // The selected line is what typing replaces.
  await page.keyboard.type('Q');
  await expect.poll(() => activeMathLatex(mathField)).toBe('Q');
});
