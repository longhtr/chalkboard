...
- Move, resize, and turn what you draw. A handle above the selection rotates it,
  and holding Shift snaps the angle to fifteen degrees.
...
To exercise registration or password recovery without sending email:

1. use an address ending in `@chalkboard.test`;
2. complete the visible local human-verification checkbox;
3. open <http://localhost:5173/development/emails>;
4. copy the eight-digit code from the captured message body.

The development inbox is memory-only, accepts no real destination, retains at most 20 messages, and disappears when the API restarts.
...
Never commit a real `.env`, and keep its values out of logs, screenshots, and bug reports.

When a call to an external provider fails, the server logs neither the raw response nor a single opaque hash. It keeps a diagnostic detailed enough to find the cause but holding no private values.

If you are investigating one, keep its completeness and correlation fields intact before changing anything: they are what distinguish a field that was genuinely missing from one that simply was not read. See [Bounded failure evidence](docs/architecture.md#bounded-failure-evidence) and the [account-email runbook](deploy/account-email-operations.md#provider-failure-evidence-and-diagnosis).
...
| `deploy`          | Production Compose, TLS, backup, account-email infrastructure, materialization, and runbooks  |
...
- A turn is stored as an angle only. Stored geometry always describes the shape
  standing upright, and everything that draws, clicks, crops, or exports applies
  that angle for itself.
...
- The current source includes five shared, writable demo accounts for environments where normal email registration cannot be exercised. Their credentials and content are public, their writes have strict per-identity and aggregate limits, and their cloud data and sessions reset daily at 00:00 UTC. Never use them for private data.
...
Normal users can permanently delete their account and cloud data from Account settings.
