...
An element that has been turned stores an angle and nothing more. Its stored
geometry always describes the shape standing upright; the renderer turns the
canvas around the middle of that upright box and draws the ordinary shape into
it. Every consumer therefore applies the angle for itself, and there is one
place -- `elementRotationCenter` -- that says which point to turn around. Two
consumers deriving that point differently is how a shape ends up drawn in one
place and clickable in another. `rotatedElementBounds` gives the upright box
around what an element covers once turned, and anything that sizes, crops, or
culls around content uses that rather than the stored box.
...
Source view and the rendered field spell the same document differently. A line break is a `\n` character in source and an invisible marker in the field. `mixedMath` keeps the list of what each marker means, and everything that needs it reads that list rather than keeping its own copy.

The code never assumes those two spellings line up correctly. An old saved file or a new version of MathLive can break that. It enforces the rules directly instead:

- Switching views keeps positions in order, and gives each one its own spot while the document has room. That is what makes every position reachable.
- Clicking works out which row you clicked first, then keeps the answer inside that row. Skip that and a click past the end of a short row lands on the row below, because that is where the nearest character is.
...
Whatever decides that a message may be sent is kept in PostgreSQL, so a restart cannot reset it. The limits are keyed by HMAC digests rather than by raw client addresses or unknown reset destinations, so the counters work without storing who anyone is.

- **Sending.** A global reservation, taken under a transaction lock, stays below the configured limits and below immutable hard caps of 100 a day and 3,000 a rolling month, which match the provider free plan's allowance.
- **Accounts.** The database starts with all email flows disabled, and caps verified normal accounts at 250. That ceiling is enforced twice, in configuration and by a database CHECK constraint.
- **Ambiguity counts against you.** A reserved or ambiguous intent is held for the full rolling month, because it may represent a message the provider actually accepted.
- **Retries are free.** After the human and address checks, a registration retry that finds a live pending record returns that record instead of admitting again. Its code is never rotated, and no second provider allowance is spent.

Browser tests backed by PostgreSQL get a multiplier on admission, and only on admission, because every synthetic account shares one loopback address. Development and production always use the fixed IP, destination, and account limits documented above; no environment setting can widen them.
...
Feedback from the provider arrives over a separate public boundary, and it is the one route that receives its body **unparsed**. That is deliberate: the signature covers the exact bytes, so parsing first would destroy the thing being verified. An encapsulated content-type parser keeps the raw payload for this route alone, while every other route still gets parsed JSON.

Verification happens before anything else is read. The server checks the Svix HMAC-SHA256 signature over the message identifier, timestamp, and raw body; compares every offered signature in constant time; and refuses a timestamp outside a five-minute window or a repeated signed header. Only then does it parse the minimum provider metadata.

Events are idempotent, and reconcile even when feedback arrives before the send result has been recorded. A permanent bounce or a complaint suppresses the destination, and any complaint disables registration outright.

Note what does _not_ protect this route: browser-origin checks. The endpoint signing secret is the only thing authenticating it.

Two secrets are kept in Secrets Manager, deliberately split:

- one holds an AWS-generated admission key and the operator-entered Turnstile validation value;
- the other holds the operator-entered email-provider API key and webhook signing secret.

They are separate because provider credentials can be rotated freely, while rotating the admission key resets the live abuse counters.

Getting those values onto the host never exposes them. A checksum-pinned `asm-exec` child resolves the dynamic references without ever returning a value to the agent. The host materializer then validates every value, writes the key generation and all four private values into one complete, fsynced release, and activates it atomically. A release is either complete or absent, so the server can never start holding material from two different rotations. Rotating repeatedly keeps only the current release and one predecessor, for rollback.

The cache is root-owned and mode `0700`. The five files inside are mode `0600` and owned by the verified non-root server UID, and Compose mounts them read-only into the server alone.

If that material is missing, stale, revoked, or unsafe, the flows that send email fail closed. Login, existing sessions, boards, collaboration, health, PostgreSQL, and backups all stay up.

Delivery goes through Resend over HTTPS. There is no AWS SDK and no ambient cloud credential anywhere on the send path.

The durable send-intent UUID doubles as the provider's idempotency key, and that is what makes exactly one retry safe after an ambiguous transport failure: replaying the request inside the provider's 24-hour window returns the stored result of the first attempt instead of delivering a second message. An explicit refusal, by contrast, is never retried. Engagement tracking stays switched off at the provider.

The tracked CloudFormation control owns only the two Secrets Manager objects and the exact runtime read permission. It grants no permission to send, because delivery authenticates with an API key rather than an IAM principal.

Runtime diagnostics report only whether the provider and its material are available, plus the flow booleans. Never values, destinations, codes, or event bodies.
...
Every unexpected server failure passes through one diagnostic boundary. It reads each mutable core error property exactly once — read them twice and the fingerprint could describe a different snapshot than the lengths do — and keeps one internally consistent record:

- a SHA-256 fingerprint, the error type, an allowlisted system or database code, and the HTTP status;
- the exact message and stack byte and character lengths;
- a bounded transformed summary and frame inventory;
- nested causes and aggregate failures.

Anything left out says so. Omissions for depth, cycles, unreadable properties, aggregate members, and frames are all explicit. When a transaction cleans up, both the original operation failure and any rollback failure are preserved.

These are never valid structured-log fields: raw `Error` objects, request bodies, URLs, destinations, addresses, codes, tokens, provider payloads, and user content.

When something goes wrong at an external provider, we want to keep enough to diagnose it without keeping anything private. Logging the raw response would keep too much; hashing the whole thing would keep too little. Instead each field goes through an information-preserving transformation, which means it is transformed according to what it is.

Kept exactly, because none of it is private:

- adapter-owned correlation identifiers, booleans, numbers, configuration names, Regions, actions, statuses, counts, timestamps, field names, and protocol stages.

Replaced by a fingerprint, because the value could contain private data:

- a private string, number, or binary value keeps its full SHA-256, UTF-8 byte length, value kind, classification, and completeness, but not prose anyone could read back.
- fields a provider adds in future keep bounded names or name fingerprints, scalar classifications, nested object/array structure, and explicit observed/omitted counts, without invoking accessors.

For an HTTP response specifically, the evidence inventories the response and every bounded header name, omits authentication and cookie values, applies strict grammars before retaining nominally safe values, fingerprints every other header, reason, and URL value, and keeps raw-body correlation separate from bounded JSON/XML/PEM structure. Body evidence records UTF-8 validity, whether the declared and observed byte lengths agree, whether the value is complete or a prefix, XML code and request-ID observed and omitted counts, stream-read failure, and whether cancellation was requested.

Two rules stop this from lying about itself. Cancellation is best-effort cleanup and is never awaited on a user path, so `requested-unobserved` means cancellation was invoked but its provider-controlled promise was deliberately not allowed to delay the result. And `null`, `unavailable`, `omitted`, `invalid-utf8`, `truncated`, complete-value, and prefix-only are all distinct states: a missing field may never be described as a complete one.
...
Fastify's raw request logger is switched off. In its place, one application-owned completion record holds only the request ID, method, matched route template, status, and duration. An unmatched path is never echoed back into a log.

Unexpected HTTP responses carry the same `x-request-id`, and a browser `ApiError` keeps only a validated request ID, so an operator can correlate the two.

Fatal browser recovery captures the exception once, then stores a bounded redacted diagnostic: exact character and UTF-8 byte lengths, stack topology, and an asynchronous fingerprint marked explicitly as covering a complete value or a bounded prefix. Paths are templated to `/boards/:boardId` and `/local/:boardId`, static routes are allowlisted, and anything unrecognised is recorded as `unmatched`. It never stores or renders the original query string, hash, concrete path identifier, or an unbounded message or stack.

That makes five external boundaries in total: the four server ones above, plus Cloudflare's browser script and widget, which is separate from server-side Siteverify.

The browser one keeps at most 20 local-only lifecycle records, covering script load, error, and timeout; a missing API; render or removal failure; completion; an invalid token callback; provider error or timeout; and expiry.

A record holds the stage, action, attempt, timing, and browser state; exact exception character and UTF-8 byte lengths; fixed stack-frame topology placeholders; whether a fingerprint covers a complete value or a prefix; the script host and path; and a syntactically bounded provider error code. Every stored record is rebuilt through an exact allowlist before reuse, so unknown or malformed localStorage fields are simply discarded.

A record never contains a token, site key, widget ID, raw callback value, URL query, arbitrary exception prose, or a provider-controlled stack frame, and it is never uploaded.

Input that is simply invalid gets a fixed, public-safe rejection. It is not an operational alert, because it is not a fault.

A provider, network, or database failure must never be collapsed into that same rejection, and the status codes keep the two apart: a transient failure while processing authenticated feedback returns `503`, so the provider is not falsely told we accepted it, while an invalid payload or signature returns `400`.

Failures in delivery, Turnstile, DNS, materialization, maintenance, collaboration, readiness, startup, shutdown, and command entry all keep whatever bounded evidence applies to them, and none of that changes their fail-closed behaviour.

Finally, the diagnostic boundary itself must hold. Unknown thrown values, hostile or revoked proxies, getters, serializers, invalid UTF-8, contradictory length metadata, response streams, and cancellation failures cannot escape it, delay it, or break it.
...
The two application-wide numbers are not chosen, they are derived from the disk.

Board content lives in PostgreSQL, so every byte of it lands in each daily dump — and the dumps share a volume with the data they protect. At 14-day retention the volume has to hold the content plus roughly ten times it in dumps, which puts the safe ceiling near 1.2 GB out of about 14 GB usable.

**So the content cap and the backup retention are one decision, not two.** Raise either without lowering the other and the disk fills. Worse, it fills as a cliff rather than a slope: everything is fine until suddenly nothing is.

The 5,000-board figure is 250 accounts times the
