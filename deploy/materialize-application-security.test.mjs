...
const resendApiKey = 'test-only-resend-api-key';
const resendWebhookSecret = `whsec_${Buffer.alloc(32, 5).toString('base64')}`;
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        CHALKBOARD_MATERIALIZED_RESEND_API_KEY:
          options.resendApiKey ?? resendApiKey,
        CHALKBOARD_MATERIALIZED_RESEND_WEBHOOK_SECRET:
          options.resendWebhookSecret ?? resendWebhookSecret,
...
function runRefresh(directory, cacheDir, generation = 1) {
  return spawnSync(
    ...
    [
      ...
      'arn:aws:secretsmanager:ap-southeast-1:000000000000:secret:provider-abcdef',
      'ap-southeast-1',
      cacheDir,
      String(generation),
...
test('atomically creates one complete private cache and makes identical refresh a no-op', () => {
  ...
  assert.equal(
    readFileSync(currentFile(cacheDir, 'resend-api-key'), 'utf8'),
    resendApiKey,
  );
  assert.equal(
    readFileSync(currentFile(cacheDir, 'resend-webhook-secret'), 'utf8'),
    resendWebhookSecret,
  );
  for (const name of [
    'admission-hmac-key',
...
case "$CHALKBOARD_MATERIALIZED_RESEND_API_KEY" in
  '{{resolve:secretsmanager:'*) ;;
  *) echo 'unresolved provider reference' >&2; exit 3 ;;
esac
...
export CHALKBOARD_MATERIALIZED_RESEND_API_KEY='${resendApiKey}'
export CHALKBOARD_MATERIALIZED_RESEND_WEBHOOK_SECRET='${resendWebhookSecret}'
exec "$@"
`,
  );
