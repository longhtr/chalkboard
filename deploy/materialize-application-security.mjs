...
/**
 * Every private file in one release, in a fixed order. A release is complete or
 * absent: a partially written set would let the server start with material from
 * two different rotations.
 */
function releaseContents(options, material) {
  return [
    [FILE_NAMES.admissionKey, material.admissionKey],
    [FILE_NAMES.generation, Buffer.from(String(options.generation), 'utf8')],
    [FILE_NAMES.turnstileSecret, material.turnstileSecret],
    [FILE_NAMES.resendApiKey, material.resendApiKey],
    [FILE_NAMES.resendWebhookSecret, material.resendWebhookSecret],
  ];
}
...
function validateMaterial(environment) {
  ...
  const resendApiKey = environment.CHALKBOARD_MATERIALIZED_RESEND_API_KEY;
  const resendWebhookSecret =
    environment.CHALKBOARD_MATERIALIZED_RESEND_WEBHOOK_SECRET;
  if (
  ...
  if (
    ...
    resendApiKey.length > 2_048 ||
    /[\0\r\n]/u.test(resendApiKey)
  ) {
    fail('email provider API material is invalid');
  }
  // The signing secret must decode to a usable HMAC key before it reaches the
  // server, so an unusable value fails here rather than at the first webhook.
  const encodedWebhookSecret = resendWebhookSecret?.startsWith('whsec_')
    ? resendWebhookSecret.slice(6)
    : resendWebhookSecret;
  if (
    resendWebhookSecret === undefined ||
    resendWebhookSecret.length < 8 ||
    resendWebhookSecret.length > 2_048 ||
    /[\0\r\n]/u.test(resendWebhookSecret) ||
    !/^[A-Za-z0-9+/]+={0,2}$/u.test(encodedWebhookSecret) ||
    Buffer.from(encodedWebhookSecret, 'base64').byteLength < 16
  ) {
    fail('email provider webhook material is invalid');
  }
  return {
    admissionKey: Buffer.from(admissionKey, 'utf8'),
    turnstileSecret: Buffer.from(turnstileSecret, 'utf8'),
    resendApiKey: Buffer.from(resendApiKey, 'utf8'),
    resendWebhookSecret: Buffer.from(resendWebhookSecret, 'utf8'),
  };
}
...
function alreadyCurrent(options, material) {
  ...
  return releaseContents(options, material).every(([name, contents]) =>
    privateFileMatches(
      join(release, name),
      contents,
      options.runtimeUid,
...
function materialize(options, material) {
  ...
  try {
    for (const [name, contents] of releaseContents(options, material)) {
      writePrivateFile(
        join(stagingPath, name),
        contents,
        options.runtimeUid,
        options.runtimeGid,
      );
