...
| `ses-feedback.yaml`                         | Defines the retained application secret, exact SES runtime IAM, SES configuration, and feedback               |
...
| `EMAIL_CONFIGURATION_SET`    | Configuration set applied to every transactional message |
| `EMAIL_DAILY_SEND_LIMIT`     | Canary capacity, at most the hard limit of 80            |
| `EMAIL_FROM`                 | Exact approved `Chalkboard <support@chalkboard.space>`   |
| `EMAIL_MONTHLY_SEND_LIMIT`   | Canary capacity, at most the hard limit of 2,400         |
| `EMAIL_REPLY_TO`             | Exact monitored `support@chalkboard.space` address       |
| `SES_FEEDBACK_TOPIC_ARN`     | Exact SNS topic accepted by signature verification       |
...
Do not put AWS access keys, Turnstile private values, admission HMAC material, codes, or provider tokens in `.env`. The server obtains temporary AWS credentials from the EC2 instance role through IMDSv2. The role needs the SES actions used by the application and the managed-node permissions required by Systems Manager.

Production email admission requires one complete materialized cache: the admission HMAC key, its positive generation marker, and the Turnstile validation value. The server reads bounded files from the configured paths, rejects group/other-readable or wrong-owner files, and keeps core login, boards, collaboration, health, and backups available if materialization is absent. The tracked resolver, checksum, materializer, tests, and read-only server-only mounts contain no values. Follow [`account-email-operations.md`](account-email-operations.md) for approved runtime resolution, rotation, revocation, rollback, outage, canary, and SES outcome procedures. Do not enable an email flow before that cache and the authenticated feedback path are reviewed and tested.

`SNS_CONFIRM_SUBSCRIPTION` defaults to `false`. Set it to `true` only for an explicitly approved ownership-confirmation window, against the exact configured topic, then immediately restore `false` and recreate the server.
...
`ses-feedback.yaml` is deliberately staged in two steps. It creates the retained application-security secret, an exact runtime policy scoped to the existing verified `chalkboard.space` SES identity and configuration set plus one exact verified recipient identity only while the Region remains sandboxed, a configuration set with VDM engagement disabled and no open/click tracking, and a signature-version-2 SNS topic whose policy permits only the exact SES configuration set with source-account and source-ARN conditions. It deliberately contains no `AWS::SES::EmailIdentity` resource, so it cannot import, replace, or take ownership of the existing root identity. The existing Easy DKIM remains unchanged. During the approved edge-closed cutover, `mail.chalkboard.space` is configured directly on that identity as fail-closed custom MAIL FROM after its exact DNS records resolve, and SES email forwarding is disabled in favor of the authenticated feedback path. The event destination covers send, reject, bounce, complaint, delivery, rendering-failure, and delivery-delay. The application accepts the exact topic ARN, validates the regional signing-certificate URL, verifies the SNS signature before parsing the SES message, stores minimum provider metadata, suppresses permanent bounces and complaints, and disables registration on any complaint.

The topic intentionally has no customer-managed KMS key: such a key creates a recurring charge and conflicts with this deployment's zero-incremental-service-cost constraint. This is a documented exception to the usual encrypted-topic recommendation. Do not add sensitive application content to SNS messages or expand the topic's publishers. Reassess this exception if the cost constraint changes.
...
2. create the `chalkboard-account-email` stack with `CreateHttpsSubscription=false`, the existing instance-role name, `SendingIdentityDomain=chalkboard.space`, the exact separately verified recipient as the no-echo `SandboxRecipientEmailIdentity` only while the Region remains sandboxed, and the exact public feedback endpoint; its SES configuration set is also named `chalkboard-account-email`, while its inline policy on `chalkboard-server` is named `account-email`;
3. read the configuration-set, topic, retained-secret, approved-sender, and custom-MAIL-FROM outputs without exposing unrelated account data; leave the existing root Easy DKIM unchanged and do not let the stack own that identity;
4. install `Chalkboard <support@chalkboard.space>` as `EMAIL_FROM`, `support@chalkboard.space` as `EMAIL_REPLY_TO`, the other non-secret outputs, and approved private runtime material; during the coordinated cutover publish and verify `mail.chalkboard.space`, configure it on the existing identity with `REJECT_MESSAGE`, disable SES email forwarding, deploy with every database email-flow switch still disabled, and verify `/api/email-feedback/ses` rejects unsigned input;
5. open one controlled confirmation window by setting `SNS_CONFIRM_SUBSCRIPTION=true` and recreating the stopped server and web together under the single-server procedure;
6. update the stack with `CreateHttpsSubscription=true`; only the exact signed topic/token URL may be followed;
7. restore `SNS_CONFIRM_SUBSCRIPTION=false`, recreate server and web together, and verify the subscription remains confirmed;
8. perform one explicitly approved controlled delivery, prove delivery/bounce/complaint event authentication and idempotency, then enable only the intended canary flow.

Never use raw SNS delivery, auto-confirm an arbitrary URL, log an envelope, or expose a subscription token. Keep the registration emergency switch off after a complaint until the event is investigated.
...
4. Run cloud browser tests, both Arm64 container image smoke tests, dependency audits, and CloudFormation validation for `ses-feedback.yaml`.
...
1. Verify the complete staged `SHA256SUMS`. Create a mode-`0600` staged copy of `.env`, preserve every existing value, and change only the release commit/version plus the separately approved non-secret account-registration ceiling, SES configuration-set name, sender, Reply-To, feedback-topic ARN, Turnstile site key, and send canary limits. Never print either file. Validate the staged Compose file with `--env-file <staged-env> ... config --quiet`; leave the live `.env` untouched.
...
5. provider-specific reconstruction: every SES action/resource occurrence plus Smithy metadata and accepted-send reconciliation, Turnstile server idempotency/match evidence plus the local browser lifecycle, DNS fallback/sibling/resolver context, or SNS envelope/signature/certificate/confirmation/authenticated-payload evidence; and
6. the live source/configuration/policy readback that governed that exact request.

Repeated matching fingerprints are one incident, not independent evidence. A fingerprint proves two retained raw inputs were byte-identical; it does not reveal or validate their hidden values. A private value's fingerprint does not replace safe correlation IDs, parsed structure, occurrence counts, field inventories, or policy reconstruction. Stream cancellation is best-effort cleanup and must never delay a request; `requested-unobserved` means it was invoked without awaiting the provider-controlled promise. If any complete-value flag is false, UTF-8 is invalid, declared and observed lengths disagree, an observed/omitted count is nonzero, a stream cancellation/read failed, a cancellation outcome is unobserved, a summary/inspection is truncated, a request ID is absent, an arbitrary field is unavailable, or classification does not match the request, state that exact uncertainty and stop before a speculative mutation. Preserve the live before-state and rollback material, reproduce through simulation or an isolated test where possible, and mutate production only after the complete request has been modeled.
...
Turnstile's browser script/widget is a separate provider boundary. Before reloading a failed form, preserve only the bounded local records from `chalkboard:turnstile-provider-diagnostics` through browser developer tools or a local support view; do not export all local storage. The record retains at most 20 script/widget stages, actions, attempts, timing, browser state, exact exception character/UTF-8 byte lengths, fixed stack topology, explicit complete-value or bounded-prefix fingerprint state, and safe callback code. Its reader reconstructs only allowlisted fields and discards unknown localStorage content. It contains no token, site key, widget ID, URL query, raw callback, arbitrary exception prose, or provider-controlled stack frame and is never uploaded. Clear that one record after resolution.
...
- one explicitly approved controlled SES delivery and authenticated feedback event when email is being enabled;
