...
Production email admission requires one complete materialized cache: the admission HMAC key, its positive generation marker, the Turnstile validation value, the provider API key, and the provider webhook signing secret. The server reads bounded files from the configured paths, rejects group/other-readable or wrong-owner files, and keeps core login, boards, collaboration, health, and backups available if materialization is absent. Provider credentials sit in a separate failure domain: if only they are missing, email delivery fails closed while human verification and board admission keep working. The tracked resolver, checksum, materializer, tests, and read-only server-only mounts contain no values. Follow [`account-email-operations.md`](account-email-operations.md) for approved runtime resolution, rotation, revocation, rollback, outage, canary, and purge procedures. Do not enable an email flow before that cache and the signed webhook path are reviewed and tested.
