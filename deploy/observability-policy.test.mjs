...
test('external provider boundaries use complete shared diagnostics and bounded logging', () => {
  ...
  const delivery = combined([
    resolve('apps/server/src/accounts/verificationEmail.ts'),
    resolve('apps/server/src/accounts/resendVerificationEmail.ts'),
  ]);
  for (const invariant of [
    ...
    assert.match(delivery, new RegExp(invariant, 'u'));
  }
  ...
  const webhook = readFileSync(
    resolve('apps/server/src/email/resendFeedback.ts'),
    'utf8',
  );
  for (const invariant of [
    'headerShape',
    ...
    'toleranceSeconds',
    'signatureEntriesObserved',
    'signatureVersionsObserved',
    'supportedSignatureEntries',
    'payloadExtraFieldNames',
    'ResendFeedbackPayloadDiagnostic',
    'parseResendFeedbackDetailed',
    'timingSafeEqual',
  ]) {
    assert.match(webhook, new RegExp(invariant, 'u'));
  }
  ...
  assert.match(providerSource, /https:\/\/api\.resend\.com\/emails/u);
  assert.match(providerSource, /challenges\.cloudflare\.com/u);
  assert.match(providerSource, /new Resolver/u);
  assert.match(providerSource, /svix-signature/u);
  // No AWS SDK may re-enter the server once delivery leaves SES.
  assert.doesNotMatch(combined(serverSources), /@aws-sdk\//u);
  ...
  assert.deepEqual(
    externalFetchFiles
      ...
      .sort(),
    [
      'apps/server/src/accounts/resendVerificationEmail.ts',
...
test('operations documentation defines evidence preservation before remediation', () => {
  ...
  assert.match(operations, /accountEmailDeliveryFailure/u);
  assert.match(operations, /fingerprint and UTF-8 byte length only/u);
  ...
  assert.match(operations, /timestamp skew against the enforced tolerance/u);
  // Certainty drives the operator's next action, so the runbook must state it.
  assert.match(operations, /Never manually retry an ambiguous send result/u);
