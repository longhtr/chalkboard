...
For the webhook preserve the complete typed `resendFeedbackFailure` or ignored-event diagnostic. This includes signed-header presence and repetition, timestamp skew against the enforced tolerance, observed/malformed signature entry counts and offered versions, payload schema issues and counts, and the names of unexpected top-level fields. No value from a verified payload is ever retained, because it carries the plaintext recipient and subject. Invalid signature, timestamp, header, body, and payload failures return `400`; authenticated database processing failure returns `503` so the provider is not falsely acknowledged and retries the delivery. Never log a webhook body, signature header, signing secret, provider ID, or recipient.
